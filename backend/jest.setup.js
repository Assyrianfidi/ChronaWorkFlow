...
jest.mock('@prisma/client', () => ({
  PrismaClient: jest.fn().mockImplementation(() => ({
    user: {
      create: jest.fn().mockResolvedValue({ id: 1, email: 'test@example.com', role: 'USER' }),
      findUnique: jest.fn().mockResolvedValue({ id: 1, email: 'test@example.com', role: 'USER' }),
      findMany: jest.fn().mockResolvedValue([]),
      update: jest.fn().mockResolvedValue({ id: 1, email: 'test@example.com', role: 'USER' }),
...
console.log('Test environment initialized with NODE_ENV:', process.env.NODE_ENV);
