...
async function testUserRepository() {
  try {
    ...
    const deletedUser = await db.select().from(users).where(eq(users.id, createdUser.id));
...
async function runTests() {
  ...
  console.log('\n📊 Test Results:');
  console.log(`- Database Connection: ${connectionSuccess ? '✅ PASSED' : '❌ FAILED'}`);
  console.log(`- User Repository: ${repositorySuccess ? '✅ PASSED' : '❌ FAILED'}`);
