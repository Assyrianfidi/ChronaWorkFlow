...
var Validator = /** @class */ (function () {
     ...
     * Validate an object
     */
    Validator.prototype.validateObject = function (data, schema, prefix, errors, validatedData) {
        var _a;
        for (var _i = 0, _b = Object.entries(schema); _i < _b.length; _i++) {
            var _c = _b[_i], field = _c[0], rule = _c[1];
            ...
            if (this.isValidationRule(rule)) {
                ...
                if (value && typeof value === 'object') {
                    validatedData[field] = {};
                    this.validateObject(value, rule, fieldPath, errors, validatedData[field]);
                }
                else if ((_a = rule[field]) === null || _a === void 0 ? void 0 : _a.required) {
                    errors.push({
                        field: fieldPath,
     ...
     */
    Validator.prototype.validateField = function (value, rule, fieldPath, errors, validatedData) {
        ...
        if (typeof value === 'string') {
            if (rule.minLength && value.length < rule.minLength) {
                errors.push({
                    field: fieldPath,
                    message: "".concat(fieldName, " must be at least ").concat(rule.minLength, " characters long"),
                    value: value
                });
            }
            if (rule.maxLength && value.length > rule.maxLength) {
                errors.push({
                    field: fieldPath,
                    message: "".concat(fieldName, " must be no more than ").concat(rule.maxLength, " characters long"),
                    value: value
                });
            }
        ...
        if (typeof value === 'number') {
            ...
            if (rule.max !== undefined && value > rule.max) {
                errors.push({
                    field: fieldPath,
                    message: "".concat(fieldName, " must be no more than ").concat(rule.max),
                    value: value
                });
            }
        }
        ...
        if (rule.custom) {
            ...
            if (customResult !== true) {
                errors.push({
                    field: fieldPath,
                    message: typeof customResult === 'string' ? customResult : "".concat(fieldName, " is invalid"),
                    value: value
                });
            }
        }
        // If no errors, add to validated data
        if (errors.filter(function (e) { return e.field === fieldPath; }).length === 0) {
    ...
    Validator.prototype.isSchema = function (rule) {
        return rule && typeof rule === 'object' && !Array.isArray(rule) && Object.keys(rule).some(function (key) {
            var value = rule[key];
