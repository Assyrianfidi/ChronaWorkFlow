 ...
 * @access  Private (Admin/Manager only)
 */
router.post('/alerts/:alertId/acknowledge', authenticate, (req, res) => {
  try {
    // Check permissions
    if (!['admin', 'manager'].includes(req.user.role)) {
      return res.status(403).json({
    ...
    const success = MonitoringService.acknowledgeAlert(req.params.alertId, req.user.id);
 ...
 * @access  Private (Admin only)
 */
router.get('/security-alerts', authenticate, (req, res) => {
  try {
    // Check permissions - Admin only
    if (req.user.role !== 'admin') {
      return res.status(403).json({
 ...
 * @access  Private (Admin only)
 */
router.post('/security-alerts/:alertId/acknowledge', authenticate, (req, res) => {
  try {
    // Check permissions - Admin only
    if (req.user.role !== 'admin') {
      return res.status(403).json({
    ...
    const success = AuditLoggerService.acknowledgeAlert(req.params.alertId, req.user.id);
 ...
 * @access  Private (Admin/Manager only)
 */
router.get('/audit-stats', authenticate, (req, res) => {
  try {
    // Check permissions
    if (!['admin', 'manager'].includes(req.user.role)) {
      return res.status(403).json({
 ...
 * @access  Private (Admin only)
 */
router.get('/performance-report', authenticate, (req, res) => {
  try {
    // Check permissions - Admin only
    if (req.user.role !== 'admin') {
      return res.status(403).json({
 ...
 * @access  Private (Admin only)
 */
router.get('/export-audit-logs', authenticate, (req, res) => {
  try {
    // Check permissions - Admin only
    if (req.user.role !== 'admin') {
      return res.status(403).json({
    ...
    const filters = {
      ...
      startDate: req.query.startDate,
      endDate: req.query.endDate,
      period: req.query.period ? { days: parseInt(req.query.period) } : undefined
    };
...
router.get('/dashboard', authenticate, (req, res) => {
  try {
    ...
    const recentSecurityAlerts = AuditLoggerService.getSecurityAlerts({ limit: 10 });
