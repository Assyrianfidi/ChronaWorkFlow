...
import { validateEnv, server } from "./config/env";
const { NODE_ENV, PORT, CORS_ORIGIN } = server;
...
app.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    const healthData = {
      status:
        systemHealth.isHealthy && dbStatus === "healthy"
          ? "healthy"
          : "unhealthy",
...
const fs = require("fs");
const route = (mountPath, relPath: any) => {
...
if (NODE_ENV === "development") {
  try {
    ...
    app.get("/test-email", async (req: any, res: any) => {
      try {
        const to =
          req.query.to || process.env.DEV_TEST_EMAIL || "test@example.com";
        await sendEmail("welcome", to, "Welcome to AccuBooks (test)", {
          name: "Test User",
