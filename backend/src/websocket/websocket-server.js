...
var WebSocketServer = /** @class */ (function (_super) {
    ...
    function WebSocketServer(options) {
        ...
        _this.rooms = new Map();
        _this.options = __assign({ port: 8080, path: '/ws', heartbeatInterval: 30000, maxConnections: 1000, enableCompression: true }, options);
    ...
    WebSocketServer.prototype.sendToClient = function (clientId, message) {
        ...
        try {
            var messageData = __assign(__assign({}, message), { timestamp: Date.now(), id: message.id || this.generateMessageId() });
            ...
            console.error("Failed to send message to client ".concat(clientId, ":"), error);
     ...
     */
    WebSocketServer.prototype.sendToRoom = function (roomName, message, excludeClientId) {
    ...
    WebSocketServer.prototype.broadcastToRole = function (role, message) {
        ...
        for (var _i = 0, _b = this.clients.values(); _i < _b.length; _i++) {
            var client = _b[_i];
            if (((_a = client.metadata) === null || _a === void 0 ? void 0 : _a.role) === role) {
                if (this.sendToClient(client.id, message)) {
                    sentCount++;
                }
            }
    ...
    WebSocketServer.prototype.getClientsInRoom = function (roomName) {
        ...
        return Array.from(room)
            .map(function (clientId) { return _this.clients.get(clientId); })
    ...
    WebSocketServer.prototype.getStats = function () {
        ...
        for (var _i = 0, _a = this.rooms.entries(); _i < _a.length; _i++) {
            var _b = _a[_i], roomName = _b[0], room = _b[1];
        ...
        if (typeof client.socket.on === 'function') {
            client.socket.on('message', function (data) {
                try {
                    ...
                    console.error("Invalid message from client ".concat(client.id, ":"), error);
                }
            });
            // Close handler
