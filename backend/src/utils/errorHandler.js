 ...
 */
var createErrorResponse = function (
  code,
  message,
  details,
  statusCode,
...
var globalErrorHandler = function (err, req, res, next) {
  ...
  // Send error response
  var errorResponse = (0, exports.createErrorResponse)(
    errorCode,
    error.message,
    details,
    statusCode,
...
var notFoundHandler = function (req, res, next) {
  var error = new ApiError(
    "Route ".concat(req.originalUrl, " not found"),
    404,
...
var SystemPanicMonitor = /** @class */ (function () {
  ...
  SystemPanicMonitor.checkSystemHealth = function () {
    ...
    if (heapUsed > this.panicThresholds.memoryUsage) {
      issues.push(
        "High memory usage: ".concat((heapUsed * 100).toFixed(2), "%"),
      );
    }
    // Check error rate
    ...
    if (
      ...
      issues.push(
        "High error rate: ".concat((errorRate * 100).toFixed(2), "%"),
      );
    }
    // Check response time
    if (this.metrics.avgResponseTime > this.panicThresholds.responseTime) {
      issues.push(
        "High average response time: ".concat(
          this.metrics.avgResponseTime.toFixed(2),
          "ms",
        ),
      );
    }
