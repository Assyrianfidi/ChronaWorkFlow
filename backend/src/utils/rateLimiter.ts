...
export class RateLimiter {
  ...
  static perIPAdaptive(req: Request, res: Response, next: NextFunction) {
    ...
    this.checkIPReputation(ip)
      .then((reputation) => {
        ...
        (CacheEngine as any)
          .checkRateLimit(identifier, limits.requests, limits.window)
          .then((result: any) => {
            res.set({
   ...
   */
  static burstControl(maxBurst: number, cooldownSeconds: number) {
    return async (req: Request, res: Response, next: NextFunction) => {
...
export class RateLimitMiddleware {
   ...
   */
  static byUserTier(
    tierLimits: Record<string, { requests: number; window: number }>,
  ) {
    return async (req: Request, res: Response, next: NextFunction) => {
