...
var CircuitBreaker = /** @class */ (function () {
    ...
    this.options = __assign(
      {
        ...
        expectedErrorRate: options.expectedErrorRate || 0.5,
        ...
        switch (_a.label) {
          case 0:
            ...
            if (this.state === CircuitState.OPEN && now >= this.nextAttempt) {
              ...
              this.halfOpenCalls = 0;
              console.log(
                "[CIRCUIT_BREAKER] ".concat(
                  this.service,
                  ": Opening to HALF_OPEN state",
                ),
              );
            }
            ...
            if (this.state === CircuitState.OPEN) {
              throw new Error(
                "Circuit breaker OPEN for "
                  .concat(this.service, ". Next attempt at ")
                  .concat(new Date(this.nextAttempt).toISOString()),
              );
            }
            ...
            if (
              ...
              throw new Error(
                "Circuit breaker HALF_OPEN for ".concat(
                  this.service,
                  ". Max calls exceeded.",
                ),
              );
  ...
  CircuitBreaker.prototype.onSuccess = function (now) {
    ...
    if (this.state === CircuitState.HALF_OPEN) {
      ...
      if (this.halfOpenCalls >= this.options.halfOpenMaxCalls) {
        ...
        console.log(
          "[CIRCUIT_BREAKER] ".concat(
            this.service,
            ": Circuit CLOSED after successful half-open period",
          ),
        );
      }
    }
    console.log(
      "[CIRCUIT_BREAKER] "
        .concat(this.service, ": Success recorded. State: ")
        .concat(this.state),
    );
  };
  /**
  ...
  CircuitBreaker.prototype.onFailure = function (now) {
    ...
    if (this.state === CircuitState.HALF_OPEN) {
      ...
      console.log(
        "[CIRCUIT_BREAKER] ".concat(
          this.service,
          ": Circuit OPEN after failure in half-open state",
        ),
      );
      ...
      if (
        ...
        console.log(
          "[CIRCUIT_BREAKER] "
            .concat(this.service, ": Circuit OPEN. Failures: ")
            .concat(this.failures, ", Error Rate: ")
            .concat((errorRate * 100).toFixed(2), "%"),
        );
      }
    }
    console.log(
      "[CIRCUIT_BREAKER] "
        .concat(this.service, ": Failure recorded. State: ")
        .concat(this.state, ", Total Failures: ")
        .concat(this.failures),
    );
  };
  /**
  ...
  CircuitBreaker.prototype.open = function () {
    ...
    console.log(
      "[CIRCUIT_BREAKER] ".concat(this.service, ": Circuit manually OPENED"),
    );
  };
