...
var PaginationEngine = /** @class */ (function () {
    ...
    PaginationEngine.paginateWithOffset = function (model_1, options_1) {
        return __awaiter(this, arguments, void 0, function (model, options, whereClause) {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 3:
                        error_1 = _b.sent();
                        throw new errorHandler_js_1.ApiError('Failed to fetch paginated data', 500, errorHandler_js_1.ErrorCodes.DATABASE_ERROR, true, error_1 instanceof Error ? error_1.message : 'Unknown error');
    ...
    PaginationEngine.paginateWithCursor = function (model_1, options_1) {
        return __awaiter(this, arguments, void 0, function (model, options, whereClause, cursorField) {
            var limit, direction, enhancedWhere, orderBy, data, hasNext, hasPrev, pagination, error_2;
            if (whereClause === void 0) { whereClause = {}; }
            if (cursorField === void 0) { cursorField = 'id'; }
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        limit = Math.min(100, Math.max(1, options.limit || 20));
                        direction = options.direction || 'forward';
                        enhancedWhere = this.buildCursorWhereClause(whereClause, options, cursorField, direction);
                        orderBy = this.buildCursorOrderByClause(options, cursorField, direction);
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, model.findMany({
                                where: enhancedWhere,
                                orderBy: orderBy,
                    ...
                    case 3:
                        error_2 = _a.sent();
                        throw new errorHandler_js_1.ApiError('Failed to fetch cursor paginated data', 500, errorHandler_js_1.ErrorCodes.DATABASE_ERROR, true, error_2 instanceof Error ? error_2.message : 'Unknown error');
                    case 4: return [2 /*return*/];
                }
            });
        });
     ...
     */
    PaginationEngine.buildCursorWhereClause = function (baseWhere, options, cursorField, direction) {
        var _a;
        var where = this.buildWhereClause(baseWhere, options);
        if (options.cursor) {
            var cursorOperator = direction === 'forward' ? '>' : '<';
            where[cursorField] = (_a = {}, _a[cursorOperator] = options.cursor, _a);
        }
        return where;
    };
    /**
    ...
    PaginationEngine.buildOrderByClause = function (options) {
        var _a;
        var _b = options.sortBy, sortBy = _b === void 0 ? 'createdAt' : _b, _c = options.sortOrder, sortOrder = _c === void 0 ? 'desc' : _c, _d = options.nulls, nulls = _d === void 0 ? 'last' : _d;
        return [
            (_a = {},
                _a[sortBy] = sortOrder,
                _a)
     ...
     */
    PaginationEngine.buildCursorOrderByClause = function (options, cursorField, direction) {
        var _a, _b;
        var _c = options.sortBy, sortBy = _c === void 0 ? 'createdAt' : _c, _d = options.sortOrder, sortOrder = _d === void 0 ? 'desc' : _d;
        // For cursor pagination, we need consistent ordering
        var primaryOrder = direction === 'forward' ? sortOrder : sortOrder === 'asc' ? 'desc' : 'asc';
        var orderBy = [
            (_a = {},
                _a[sortBy] = primaryOrder,
                _a)
        ];
        // Add cursor field as secondary sort for uniqueness
        if (sortBy !== cursorField) {
            orderBy.push((_b = {},
                _b[cursorField] = primaryOrder,
                _b));
        }
        return orderBy;
    };
    ...
    PaginationEngine.validatePaginationParams = function (params) {
        if (params.page && (isNaN(params.page) || params.page < 1)) {
            throw new errorHandler_js_1.ApiError('Invalid page parameter. Must be a positive integer.', 400, errorHandler_js_1.ErrorCodes.INVALID_INPUT);
        }
        if (params.limit && (isNaN(params.limit) || params.limit < 1 || params.limit > 100)) {
            throw new errorHandler_js_1.ApiError('Invalid limit parameter. Must be between 1 and 100.', 400, errorHandler_js_1.ErrorCodes.INVALID_INPUT);
        }
        if (params.cursor && typeof params.cursor !== 'string') {
            throw new errorHandler_js_1.ApiError('Invalid cursor parameter. Must be a string.', 400, errorHandler_js_1.ErrorCodes.INVALID_INPUT);
        }
    };
    /**
    ...
    PaginationEngine.validateSortParams = function (params, allowedFields) {
        if (params.sortBy && !allowedFields.includes(params.sortBy)) {
            throw new errorHandler_js_1.ApiError("Invalid sort field. Allowed fields: ".concat(allowedFields.join(', ')), 400, errorHandler_js_1.ErrorCodes.INVALID_INPUT);
        }
        if (params.sortOrder && !['asc', 'desc'].includes(params.sortOrder)) {
            throw new errorHandler_js_1.ApiError('Invalid sort order. Must be "asc" or "desc".', 400, errorHandler_js_1.ErrorCodes.INVALID_INPUT);
        }
    };
    /**
     * Create pagination metadata for response
     */
    PaginationEngine.createPaginationMeta = function (pagination, baseUrl, queryParams) {
        var meta = __assign({}, pagination);
        // Add navigation links if using offset pagination
        if (pagination.page !== undefined) {
            var page = pagination.page, limit = pagination.limit, totalPages = pagination.totalPages;
    ...
    FilterEngine.validateFilters = function (filters) {
        if (filters.dateRange) {
            ...
            if (start && end && start > end) {
                throw new errorHandler_js_1.ApiError('Start date must be before end date.', 400, errorHandler_js_1.ErrorCodes.INVALID_INPUT);
            }
        }
        if (filters.numericRange) {
            var _b = filters.numericRange, min = _b.min, max = _b.max;
            if (min !== undefined && max !== undefined && min > max) {
                throw new errorHandler_js_1.ApiError('Minimum value must be less than or equal to maximum value.', 400, errorHandler_js_1.ErrorCodes.INVALID_INPUT);
            }
        }
    };
    return FilterEngine;
