      ...
      switch (model) {
        ...
        case 'Account':
          if (data.code && data.companyId) {
            ...
            const existing = await prisma.account.findFirst({ where: whereClause });
            if (existing) {
        ...
        case 'InventoryItem':
          if (data.sku) {
            ...
            const existing = await prisma.inventoryItem.findFirst({ where: whereClause });
            if (existing) {
        ...
        case 'CompanyMember':
          if (data.companyId && data.userId) {
            ...
            const existing = await prisma.companyMember.findFirst({ where: whereClause });
            if (existing) {
        ...
        case 'ReconciliationReport':
          if (data.title && data.userId) {
            ...
            const existing = await prisma.reconciliationReport.findFirst({ where: whereClause });
            if (existing) {
      ...
      switch (model) {
        ...
        case 'InventoryItem':
          ...
          if (data.costPrice && data.sellingPrice && data.sellingPrice < data.costPrice) {
            errors.push('Selling price must be greater than or equal to cost price');
          }
          break;
        ...
        case 'Transaction':
          ...
          if (data.debits && data.credits) {
            const totalDebits = data.debits.reduce((sum, debit) => sum + Number(debit.amount), 0);
            const totalCredits = data.credits.reduce((sum, credit) => sum + Number(credit.amount), 0);
        ...
        switch (model) {
          ...
          case 'Account':
            ...
            const accountTransactions = await prisma.transaction.count({
              where: {
                OR: [
                  { debitAccountId: data.id },
                  { creditAccountId: data.id }
                ]
              }
            });
