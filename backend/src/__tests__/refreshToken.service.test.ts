...
describe('RefreshTokenService', () => {
  ...
  describe('createRefreshToken', () => {
    ...
    it('should set correct expiry date', async () => {
      ...
      expectedExpiry.setDate(expectedExpiry.getDate() + RefreshTokenService.getRefreshTokenExpiry());
  ...
  describe('invalidateUserRefreshTokens', () => {
    it('should invalidate all user tokens', async () => {
      ...
      // Verify tokens exist
      expect(await prisma.refreshToken.findUnique({ where: { tokenHash: hash1 } })).toBeTruthy();
      expect(await prisma.refreshToken.findUnique({ where: { tokenHash: hash2 } })).toBeTruthy();

      // Invalidate all tokens
      await RefreshTokenService.invalidateUserRefreshTokens(testUserId);

      // All tokens should be gone
      expect(await prisma.refreshToken.findUnique({ where: { tokenHash: hash1 } })).toBeFalsy();
      expect(await prisma.refreshToken.findUnique({ where: { tokenHash: hash2 } })).toBeFalsy();
    });
  });
  ...
  describe('cleanupExpiredTokens', () => {
    it('should clean up expired tokens', async () => {
      ...
      const validToken = await RefreshTokenService.createRefreshToken(testUserId);
      ...
      expect(await prisma.refreshToken.findUnique({ where: { tokenHash } })).toBeFalsy();
      
      // Valid token should remain
      expect(await prisma.refreshToken.findUnique({ where: { tokenHash: validHash } })).toBeTruthy();
    });
