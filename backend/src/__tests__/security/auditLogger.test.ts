...
import { logAuthEvent, logDataEvent, logSecurityEvent, logAllRequests, logPerformance } from '../../middleware/security/auditLogger.middleware.js';

// Mock the services
jest.mock('../../services/auditLogger.service.js');
jest.mock('../../services/monitoring.service.js');

const MockedAuditLoggerService = AuditLoggerService as jest.Mocked<typeof AuditLoggerService>;
...
describe('Audit Logger Middleware', () => {
  ...
  beforeEach(() => {
    ...
    app.post('/auth/login', 
      ...
      (req, res) => res.json({ success: true, data: { user: { id: 1 } } })
    );
    
    app.post('/auth/login-failed',
      mockAuth,
      logAuthEvent('LOGIN', false),
      (req, res) => res.json({ success: false, message: 'Invalid credentials' })
    );
    
    app.post('/accounts',
      mockAuth,
      logDataEvent('CREATE', 'ACCOUNT'),
      (req, res) => res.json({ success: true, data: { id: 1, name: 'Test Account' } })
    );
