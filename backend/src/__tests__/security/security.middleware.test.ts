...
import applySecurityMiddlewares, { authRateLimiter, sensitiveRouteLimiter } from '../../middleware/security';
...
describe('Security Middleware', () => {
  ...
  beforeEach(() => {
    ...
    app.post('/api/v1/auth/register', (req, res) => res.json({ success: true }));
  ...
  describe('Rate Limiting', () => {
    ...
    it('should limit auth login attempts to 5 per minute', async () => {
      const promises = [];
      for (let i = 0; i < 7; i++) {
        promises.push(request(app).post('/api/v1/auth/login').send({ email: 'test@example.com', password: 'password' }));
      }
    ...
    it('should limit auth register attempts to 5 per minute', async () => {
      const promises = [];
      for (let i = 0; i < 7; i++) {
        promises.push(request(app).post('/api/v1/auth/register').send({ email: `test${i}@example.com`, password: 'password' }));
      }
    ...
    it('should limit sensitive operations to 10 per 30 seconds', async () => {
      const promises = [];
      for (let i = 0; i < 12; i++) {
        promises.push(request(app).post('/api/v1/accounts').send({ name: 'Test Account' }));
      }
