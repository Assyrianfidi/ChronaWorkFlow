...
describe("Database Security Service", () => {
  ...
  describe("Permission Checking", () => {
    it("should allow admin full access", () => {
      ...
      expect(
        DatabaseSecurityService.hasPermission(adminUser, "accounts", "read"),
      ).toBe(true);
      expect(
        DatabaseSecurityService.hasPermission(adminUser, "accounts", "write"),
      ).toBe(true);
      expect(
        DatabaseSecurityService.hasPermission(adminUser, "accounts", "delete"),
      ).toBe(true);
      expect(
        DatabaseSecurityService.hasPermission(adminUser, "users", "write"),
      ).toBe(true);
    });
    ...
    it("should allow managers appropriate access", () => {
      ...
      expect(
        DatabaseSecurityService.hasPermission(managerUser, "accounts", "read", {
          companyId: "company1",
        }),
      ).toBe(true);
      expect(
      ...
      expect(
        DatabaseSecurityService.hasPermission(managerUser, "users", "write"),
      ).toBe(false);
      expect(
        DatabaseSecurityService.hasPermission(managerUser, "users", "read"),
      ).toBe(true);
    });
    ...
    it("should allow users limited access", () => {
      ...
      expect(
        DatabaseSecurityService.hasPermission(regularUser, "accounts", "read", {
          companyId: "company1",
        }),
      ).toBe(true);
      expect(
        DatabaseSecurityService.hasPermission(regularUser, "accounts", "write"),
      ).toBe(false);
      expect(
        DatabaseSecurityService.hasPermission(
          regularUser,
    ...
    it("should enforce company ownership for accounts", () => {
      ...
      expect(
        DatabaseSecurityService.hasPermission(user, "accounts", "read", {
          companyId: "company1",
        }),
      ).toBe(true);
      expect(
    ...
    it("should reject unauthenticated users", () => {
      expect(
        DatabaseSecurityService.hasPermission(null, "accounts", "read"),
      ).toBe(false);
      expect(
        DatabaseSecurityService.hasPermission(undefined, "accounts", "read"),
      ).toBe(false);
    });
  });
  ...
  describe("Sensitive Field Access", () => {
    it("should allow admin access to all sensitive fields", () => {
      ...
      expect(
        DatabaseSecurityService.canAccessSensitiveField(admin, "password"),
      ).toBe(true);
      expect(
        DatabaseSecurityService.canAccessSensitiveField(admin, "email"),
      ).toBe(true);
      expect(
        DatabaseSecurityService.canAccessSensitiveField(admin, "salary"),
      ).toBe(true);
    });
    ...
    it("should restrict user access to sensitive fields", () => {
      ...
      expect(
        DatabaseSecurityService.canAccessSensitiveField(user, "password"),
      ).toBe(false);
      expect(
        DatabaseSecurityService.canAccessSensitiveField(user, "email"),
      ).toBe(false);
      expect(
        DatabaseSecurityService.canAccessSensitiveField(user, "salary"),
      ).toBe(false);
    });
    ...
    it("should allow managers access to some sensitive fields", () => {
      ...
      expect(
        DatabaseSecurityService.canAccessSensitiveField(manager, "password"),
      ).toBe(false);
      expect(
        DatabaseSecurityService.canAccessSensitiveField(manager, "email"),
      ).toBe(true);
      expect(
        DatabaseSecurityService.canAccessSensitiveField(manager, "salary"),
      ).toBe(true);
    });
  });
  ...
  describe("Row-Level Security Filters", () => {
    it("should return empty filter for admin", () => {
      ...
      expect(
        DatabaseSecurityService.getRowLevelSecurityFilter(admin, "Account"),
      ).toEqual({});
      expect(
        DatabaseSecurityService.getRowLevelSecurityFilter(admin, "Transaction"),
      ).toEqual({});
    });
    ...
    it("should return company filter for non-admin users", () => {
      ...
      expect(
        DatabaseSecurityService.getRowLevelSecurityFilter(user, "Account"),
      ).toEqual({
        companyId: "company1",
      });

      expect(
        DatabaseSecurityService.getRowLevelSecurityFilter(
          user,
          "InventoryItem",
        ),
      ).toEqual({
  ...
  describe("Database Constraint Validation", () => {
    it("should validate email format", () => {
      const result = DatabaseSecurityService.validateConstraints(
        "User",
        { email: "invalid-email" },
        "create",
      );
    ...
    it("should prevent deletion of records with dependencies", () => {
      const userResult = DatabaseSecurityService.validateConstraints(
        "User",
        {},
        "delete",
      );
      ...
      const accountResult = DatabaseSecurityService.validateConstraints(
        "Account",
        {},
        "delete",
      );
...
describe("Database Security Middleware", () => {
  ...
  beforeEach(() => {
    ...
    app.post("/sensitive", mockAuth, validateSensitiveFieldAccess, (req, res) =>
      res.json({ message: "sensitive data processed" }),
    );
  });
...
describe("Database Constraints Service", () => {
  ...
  describe("Unique Constraint Validation", () => {
    it("should detect duplicate emails", async () => {
      ...
      const result = await DatabaseConstraintsService.validateUniqueConstraints(
        "User",
    ...
    it("should detect duplicate account codes in same company", async () => {
      ...
      mockPrisma.account.findFirst = jest
        .fn()
        .mockResolvedValue({ id: 1, code: "CASH", companyId: "company1" });
  ...
  describe("Foreign Key Constraint Validation", () => {
    it("should detect invalid parent account", async () => {
      ...
      const result =
        await DatabaseConstraintsService.validateForeignKeyConstraints(
          "Account",
          {
            parentId: "invalid-id",
            companyId: "company1",
