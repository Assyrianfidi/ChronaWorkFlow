...
import { CacheEngine, CacheInvalidator, CacheMiddleware } from '../../../utils/cacheEngine';
...
import { PerformanceMonitor, LoadTester, DatabaseMonitor } from '../../../utils/performanceMonitor';
import { LoggingBridge } from '../../../utils/loggingBridge';

// Mock dependencies
jest.mock('../../loggingBridge');
...
describe('Performance Layer Tests', () => {
  ...
  describe('CacheEngine', () => {
    ...
    it('should handle set operation when not connected', async () => {
      await expect(CacheEngine.set('test-key', 'value', 300)).resolves.toBeUndefined();
    });
  ...
  describe('CacheInvalidator', () => {
    ...
    it('should invalidate account caches', async () => {
      ...
      expect(delPatternSpy).toHaveBeenCalledWith('transactions:account:acc123:*');
    });
  ...
  describe('CacheMiddleware', () => {
    it('should cache method results', async () => {
      ...
      expect(setSpy).toHaveBeenCalledWith('expensiveMethod:["arg1","arg2"]', 'result', 300);
    });
    ...
    it('should return cached result when available', async () => {
      ...
      const getSpy = jest.spyOn(CacheEngine, 'get').mockResolvedValue('cached-result');
  ...
  describe('RateLimiter', () => {
    ...
    it('should create per-user rate limiter with custom limits', () => {
      const middleware = RateLimiter.perUserLimit('auth', { requests: 3, window: 120 });
      expect(typeof middleware).toBe('function');
    });
  ...
  describe('PaginationEngine', () => {
    ...
    it('should validate pagination parameters', () => {
      expect(() => PaginationEngine.validatePaginationParams({ page: 0 })).toThrow();
      expect(() => PaginationEngine.validatePaginationParams({ limit: 0 })).toThrow();
      expect(() => PaginationEngine.validatePaginationParams({ limit: 101 })).toThrow();
      expect(() => PaginationEngine.validatePaginationParams({ cursor: 123 })).toThrow();
    });

    it('should validate sort parameters', () => {
      expect(() => PaginationEngine.validateSortParams({ sortBy: 'invalid' }, ['id', 'name'])).toThrow();
      expect(() => PaginationEngine.validateSortParams({ sortOrder: 'invalid' }, ['id', 'name'])).toThrow();
    });
  ...
  describe('FilterEngine', () => {
    ...
    it('should validate filters', () => {
      expect(() => FilterEngine.validateFilters({
  ...
  describe('PerformanceMonitor', () => {
    ...
    it('should monitor database queries', async () => {
      ...
      const result = await PerformanceMonitor.monitorQuery('test-query', queryFn);
    ...
    it('should monitor cache operations', async () => {
      ...
      const result = await PerformanceMonitor.monitorCache('get', 'test-key', operationFn);
  ...
  describe('LoadTester', () => {
    it('should run concurrent requests', async () => {
      ...
      const result = await LoadTester.runConcurrentRequests('https://api.example.com/test', {
        concurrency: 5,
        totalRequests: 10
      });
    ...
    it('should run stress test', async () => {
      ...
      const result = await LoadTester.runStressTest('https://api.example.com/test', {
        startConcurrency: 1,
        maxConcurrency: 3,
        stepSize: 1,
        requestsPerStep: 2,
  ...
  describe('DatabaseMonitor', () => {
    ...
    it('should get slow queries', async () => {
      ...
      const slowQueries = await DatabaseMonitor.getSlowQueries(mockPrisma, 1000);
