...
describe('Error Handler Utils', function () {
    ...
    describe('ApiError', function () {
        ...
        it('should create ApiError with custom values', function () {
            ...
            var error = new errorHandler_1.ApiError('Validation failed', 400, errorHandler_1.ErrorCodes.VALIDATION_ERROR, true, details);
    ...
    describe('createErrorResponse', function () {
        it('should create basic error response', function () {
            var response = (0, errorHandler_1.createErrorResponse)(errorHandler_1.ErrorCodes.VALIDATION_ERROR, 'Invalid input');
            expect(response.success).toBe(false);
            expect(response.error.code).toBe(errorHandler_1.ErrorCodes.VALIDATION_ERROR);
        ...
        it('should include details when provided', function () {
            var details = { field: 'email' };
            var response = (0, errorHandler_1.createErrorResponse)(errorHandler_1.ErrorCodes.VALIDATION_ERROR, 'Invalid input', details);
            expect(response.error.details).toEqual(details);
        });
        it('should include requestId when provided', function () {
            var requestId = 'req-123';
            var response = (0, errorHandler_1.createErrorResponse)(errorHandler_1.ErrorCodes.VALIDATION_ERROR, 'Invalid input', undefined, 400, requestId);
    ...
    describe('globalErrorHandler', function () {
        it('should handle ApiError correctly', function () {
            var error = new errorHandler_1.ApiError('Test error', 404, errorHandler_1.ErrorCodes.NOT_FOUND);
        ...
        it('should handle Prisma known request errors', function () {
            var error = new client_1.Prisma.PrismaClientKnownRequestError('Unique constraint failed', { code: 'P2002', clientVersion: '4.0.0', meta: { target: ['email'] } });
            (0, errorHandler_1.globalErrorHandler)(error, mockReq, mockRes, mockNext);
        ...
        it('should handle Prisma record not found errors', function () {
            var error = new client_1.Prisma.PrismaClientKnownRequestError('Record not found', { code: 'P2025', clientVersion: '4.0.0' });
            (0, errorHandler_1.globalErrorHandler)(error, mockReq, mockRes, mockNext);
            expect(mockRes.status).toHaveBeenCalledWith(404);
            expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({
                error: expect.objectContaining({
        ...
        it('should handle JWT errors', function () {
            ...
            (0, errorHandler_1.globalErrorHandler)(jwtError, mockReq, mockRes, mockNext);
    ...
    describe('notFoundHandler', function () {
        it('should create 404 error for undefined routes', function () {
            mockReq = __assign(__assign({}, mockReq), { originalUrl: '/undefined-route' });
    ...
    describe('asyncHandler', function () {
        it('should handle resolved promises', function () { return __awaiter(void 0, void 0, void 0, function () {
