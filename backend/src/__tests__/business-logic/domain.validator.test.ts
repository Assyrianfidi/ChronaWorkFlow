...
describe("Domain Validator", () => {
  ...
  describe("validateBalance", () => {
    ...
    it("should allow overdraft within limit", () => {
      const validation = DomainValidator.validateBalance(
        100,
        500,
        "CHECKING",
        true,
  ...
  describe("validateCrossCurrencyTransaction", () => {
    it("should allow same currency without exchange rate", () => {
      const validation = DomainValidator.validateCrossCurrencyTransaction(
        "USD",
        "USD",
        100,
      );
  ...
  describe("validateTransactionTiming", () => {
    it("should allow normal timing", () => {
      ...
      const validation = DomainValidator.validateTransactionTiming(
        lastTime,
        100,
      );
  ...
  describe("validateOrThrow", () => {
    it("should throw error for invalid validation", () => {
      ...
      expect(() =>
        DomainValidator.validateOrThrow(invalidValidation),
      ).toThrow();
    });
    ...
    it("should not throw for valid validation", () => {
      ...
      expect(() =>
        DomainValidator.validateOrThrow(validValidation),
      ).not.toThrow();
    });
  });
  ...
  describe("ValidationRules", () => {
    ...
    it("should validate high-value transaction with timing check", () => {
      ...
      expect(() =>
        ValidationRules.highValueTransaction(
          {
            fromAccountId: "acc1",
            toAccountId: "acc2",
    ...
    it("should validate international transfer", () => {
      expect(() =>
        ValidationRules.internationalTransfer("USD", "EUR", 100, 0.85),
      ).not.toThrow();
    });

    it("should reject international transfer without exchange rate", () => {
      expect(() =>
        ValidationRules.internationalTransfer("USD", "EUR", 100),
      ).toThrow();
    });
  });
