...
describe("Bookkeeping Rules", () => {
  describe("validateDoubleEntry", () => {
    it("should validate a balanced transaction", () => {
      ...
      expect(() =>
        BookkeepingRules.validateDoubleEntry(transaction),
      ).not.toThrow();
    });
    ...
    it("should reject transaction with less than 2 entries", () => {
      ...
      expect(() => BookkeepingRules.validateDoubleEntry(transaction)).toThrow(
    ...
    it("should reject entry that is both debit and credit", () => {
      ...
      expect(() => BookkeepingRules.validateDoubleEntry(transaction)).toThrow(
        "Transaction is not balanced. Debits: 100, Credits: 200",
      );
    });
    ...
    it("should reject entry that is neither debit nor credit", () => {
      ...
      expect(() => BookkeepingRules.validateDoubleEntry(transaction)).toThrow(
        "Transaction is not balanced. Debits: 0, Credits: 100",
      );
    });
  ...
  describe("validateAccountConstraints", () => {
    it("should allow valid transaction", () => {
      ...
      expect(() =>
        BookkeepingRules.validateAccountConstraints(
          transaction,
          accountBalances,
          accountTypes,
        ),
  ...
  describe("reverseTransaction", () => {
    it("should create reversing transaction", () => {
      ...
      const reversalDebit = reversal.entries.find(
        (e) => e.debit && e.accountId === originalDebit.accountId,
      );
      expect(reversalDebit).toBeUndefined(); // Should be credit now

      const reversalCredit = reversal.entries.find(
        (e) => e.credit && e.accountId === originalDebit.accountId,
      );
  ...
  describe("validateTrialBalance", () => {
    it("should validate balanced trial balance", () => {
      ...
      expect(() =>
        BookkeepingRules.validateTrialBalance(accountBalances),
      ).not.toThrow();
    });
  ...
  describe("createAdjustingEntry", () => {
    it("should create adjusting entry with clearing account", () => {
      ...
      const clearingEntry = transaction.entries.find(
        (e) => e.accountId === "ADJUSTMENT_CLEARING",
      );
