...
var DomainValidator = /** @class */ (function () {
  ...
  DomainValidator.validateAmount = function (validation) {
    ...
    if (decimals > validation.precision) {
      errors.push({
        ...
        message: "Amount cannot have more than ".concat(
   ...
   */
  DomainValidator.validateBalance = function (
    currentBalance,
    transactionAmount,
    accountType,
    allowOverdraft,
  ) {
    if (allowOverdraft === void 0) {
    ...
    if (!allowOverdraft && newBalance < 0) {
      errors.push({
        ...
        message: "Insufficient funds. Current: "
          .concat(currentBalance, ", Required: ")
          .concat(transactionAmount),
        value: {
   ...
   */
  DomainValidator.validateCrossCurrencyTransaction = function (
    fromCurrency,
    toCurrency,
    amount,
    exchangeRate,
   ...
   */
  DomainValidator.validateTransactionTiming = function (
    lastTransactionTime,
    amount,
  ) {
    var errors = [];
    ...
    if (amount && amount > 1000 && minutesSinceLastTransaction < 1) {
      errors.push({
        ...
        value: {
          minutesSinceLastTransaction: minutesSinceLastTransaction,
          amount: amount,
        },
      });
    }
  ...
  DomainValidator.validateOrThrow = function (validation) {
    if (!validation.isValid) {
      ...
      throw new errorHandler_js_1.ApiError(
        "Validation failed: ".concat(errorMessages),
        400,
...
exports.ValidationRules = {
  ...
  highValueTransaction: function (transaction, lastTransactionTime) {
    ...
    if (transaction.amount > 10000) {
      var timingValidation = DomainValidator.validateTransactionTiming(
        lastTransactionTime,
        transaction.amount,
      );
