...
var FraudDetector = /** @class */ (function () {
  ...
  FraudDetector.initializeRules = function () {
    this.rules = [
      ...
      // Rule 5: Round number amount (potential money laundering)
      {
        ...
        description:
          "Transaction amount is a round number (potential structuring)",
        enabled: true,
        severity: "low",
        action: "monitor",
   ...
   */
  FraudDetector.analyzeTransaction = function (
    currentTransaction,
    historicalPatterns,
  ) {
    return __awaiter(this, void 0, void 0, function () {
      ...
      return __generator(this, function (_b) {
        ...
        for (_i = 0, _a = this.rules; _i < _a.length; _i++) {
          ...
          try {
            ...
            if (alert_1) {
              ...
              if (
                this.getSeverityLevel(alert_1.severity) >
        ...
        blocked = alerts.some(function (alert) {
  ...
  FraudDetector.checkLargeAmount = function (patterns, current) {
    ...
    var avgAmount =
      userPatterns.reduce(function (sum, p) {
        return sum + p.amount;
      }, 0) / userPatterns.length;
    ...
    if (current.amount > threshold) {
      return {
        ...
        description: "Transaction amount $"
          .concat(current.amount, " is ")
          .concat(Math.round(current.amount / avgAmount), "x user average"),
  ...
  FraudDetector.checkRapidTransactions = function (patterns, current) {
    var userPatterns = patterns.filter(function (p) {
      return p.userId === current.userId && p.accountId === current.accountId;
    });
    // Check for transactions in last 5 minutes
    var fiveMinutesAgo = new Date(current.timestamp.getTime() - 5 * 60 * 1000);
    var recentTransactions = userPatterns.filter(function (p) {
      return p.timestamp > fiveMinutesAgo;
    });
  ...
  FraudDetector.checkUnusualLocation = function (patterns, current) {
    ...
    var userPatterns = patterns.filter(function (p) {
      return p.userId === current.userId && p.location;
    });
    var locations = __spreadArray(
      [],
      new Set(
        userPatterns.map(function (p) {
          return p.location;
        }),
      ),
      true,
  ...
  FraudDetector.checkUnusualDevice = function (patterns, current) {
    ...
    var userPatterns = patterns.filter(function (p) {
      return p.userId === current.userId && p.device;
    });
    var devices = __spreadArray(
      [],
      new Set(
        userPatterns.map(function (p) {
          return p.device;
  ...
  FraudDetector.checkHighVelocity = function (patterns, current) {
    var userPatterns = patterns.filter(
      function (p) {
        return (
          p.userId === current.userId &&
          p.timestamp >
            new Date(current.timestamp.getTime() - 24 * 60 * 60 * 1000)
        );
      }, // Last 24 hours
    );
  ...
  FraudDetector.checkNewAccountHighValue = function (patterns, current) {
    ...
    if (userPatterns.length === 0 && current.amount > 5000) {
      return {
        ...
        severity: "critical",
        description: "High value transaction ($".concat(
          current.amount,
          ") from new account",
        ),
        confidence: 0.8,
  ...
  FraudDetector.checkSuspiciousMerchant = function (patterns, current) {
    ...
    if (suspiciousCategories.includes(current.merchantCategory)) {
      return {
        ...
        severity: "medium",
        description: "Transaction with suspicious merchant: ".concat(
  ...
  FraudDetector.checkAccountTakeover = function (patterns, current) {
    ...
    if (suspiciousIndicators >= 2) {
      return {
        ...
        severity: "critical",
        description: "Multiple indicators of account takeover (".concat(
          ...
          factors: {
            unusualLocation:
              ...
              !patterns.some(function (p) {
                return p.location === current.location;
              }),
            unusualDevice:
              current.device &&
              !patterns.some(function (p) {
                return p.device === current.device;
              }),
  ...
  FraudDetector.getUserProfile = function (userId, patterns) {
    ...
    var merchants = __spreadArray(
      [],
      new Set(
        userPatterns
          .map(function (p) {
            return p.merchantCategory;
          })
          .filter(Boolean),
      ),
      true,
    ...
    var profile = {
      ...
      transactionFrequency:
        userPatterns.length / Math.max(1, userPatterns.length > 0 ? 30 : 30), // per day
  ...
  FraudDetector.calculateRiskScore = function (alerts, profile) {
    ...
    var maxSeverity = Math.max.apply(
      ...
      alerts.map(function (a) {
        return severityWeights[a.severity];
      }),
    );
