...
export const validateCreateInvoice = [
  ...
  body('lines.*.description').notEmpty().withMessage('Line item description is required'),
  body('lines.*.qty').isInt({ min: 1 }).withMessage('Quantity must be a positive integer'),
  body('lines.*.unitPrice').isInt({ min: 0 }).withMessage('Unit price must be a non-negative integer'),
];

export const validateUpdateInvoice = [
  body('customerId').optional().notEmpty().withMessage('Customer ID is required'),
  body('issueDate').optional().isISO8601().withMessage('Issue date must be a valid date'),
  body('dueDate').optional().isISO8601().withMessage('Due date must be a valid date'),
  body('currency').optional().isLength({ min: 3, max: 3 }).withMessage('Currency must be 3 characters'),
  body('notes').optional().isString(),
  body('lines').optional().isArray({ min: 1 }).withMessage('Lines must be an array'),
  body('lines.*.description').optional().notEmpty().withMessage('Line item description is required'),
  body('lines.*.qty').optional().isInt({ min: 1 }).withMessage('Quantity must be a positive integer'),
  body('lines.*.unitPrice').optional().isInt({ min: 0 }).withMessage('Unit price must be a non-negative integer'),
];

export const validateAddPayment = [
  body('amount').isInt({ min: 1 }).withMessage('Amount must be a positive integer'),
  body('method').isIn(['CASH', 'CREDIT_CARD', 'BANK_TRANSFER', 'CHECK', 'ONLINE']).withMessage('Invalid payment method'),
  body('transactionRef').optional().isString(),
];

export const validateListInvoices = [
  query('status').optional().isIn(['DRAFT', 'SENT', 'PAID', 'OVERDUE', 'VOIDED']).withMessage('Invalid status'),
