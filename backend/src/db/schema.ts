import { pgTable, serial, text, boolean, timestamp, integer, decimal, unique, pgEnum, jsonb, primaryKey } from 'drizzle-orm/pg-core';
...
export const userPreferences = pgTable('user_preferences', {
  id: serial('id').primaryKey(),
  userId: integer('userId').notNull().references(() => users.id, { onDelete: 'cascade' }),
  preferences: jsonb('preferences').$type<Record<string, any>>().default({}).notNull(),
  createdAt: timestamp('createdAt').defaultNow().notNull(),
  updatedAt: timestamp('updatedAt').defaultNow().notNull(),
}, (table) => ({
...
export const userRoleMappings = pgTable('user_role_mappings', 
  {
    userId: integer('userId').notNull().references(() => users.id, { onDelete: 'cascade' }),
    roleId: integer('roleId').notNull().references(() => userRoles.id, { onDelete: 'cascade' }),
