...
export const useInventoryItems = (options: InventoryQueryOptions = {}) => {
  ...
  return useQuery<InventoryItem[], Error>({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<InventoryResponse<InventoryItem>>(`/api/inventory?${params.toString()}`);
...
export const useInventoryStats = (filters: Omit<InventoryQueryOptions, 'sortBy' | 'sortDirection' | 'page' | 'pageSize'> = {}) => {
  return useQuery<InventoryStats, Error>({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<{ data: InventoryStats }>(`/api/inventory/stats?${params.toString()}`);
      return response.data.data;
    },
    staleTime: 5 * 60 * 1000, // 5 minutes
  });
...
export const useCreateInventoryItem = () => {
  ...
  return useMutation<InventoryItem, Error, Omit<InventoryItem, 'id' | 'createdAt' | 'updatedAt'>>({
    mutationFn: async (newItem) => {
      const response = await api.post<{ data: InventoryItem }>('/api/inventory', newItem);
      return response.data.data;
    },
    onSuccess: () => {
...
export const useUpdateInventoryItem = () => {
  ...
  return useMutation<InventoryItem, Error, { id: string; updates: Partial<InventoryItem> }>({
    mutationFn: async ({ id, updates }) => {
      const response = await api.patch<{ data: InventoryItem }>(`/api/inventory/${id}`, updates);
      return response.data.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [INVENTORY_QUERY_KEY] });
...
export const useBulkUpdateInventoryItems = () => {
  ...
  return useMutation<void, Error, { ids: string[]; updates: Partial<InventoryItem> }>({
...
export const useExportInventory = () => {
  return useMutation<string, Error, { filters: InventoryFilters; format: 'csv' | 'pdf' | 'excel' }>({
