...
type SortableField = 'name' | 'quantity' | 'value' | 'category' | 'sku' | 'unitCost' | 'unitPrice' | 'lastStockUpdate';
...
export function useInventoryFilters({
  ...
  const getInitialState = useCallback(() => {
    ...
    return {
      ...
      selectedCategory: Array.isArray(categoryParam) ? categoryParam[0] : categoryParam || initialFilters.categoryId || '',
      statusFilter: (params.get('status') as InventoryStatus) || initialFilters.status,
      quantityRange: {
        min: params.has('minQty') ? Number(params.get('minQty')) : initialFilters.quantityRange?.min,
        max: params.has('maxQty') ? Number(params.get('maxQty')) : initialFilters.quantityRange?.max,
      },
      sortBy: (params.get('sortBy') as keyof InventoryItem) || initialFilters.sortBy || 'name',
      sortOrder: (params.get('sortOrder') as 'asc' | 'desc') || initialFilters.sortOrder || 'asc',
      page: params.has('page') ? Number(params.get('page')) : initialFilters.page || 1,
      pageSize: params.has('pageSize') ? Number(params.get('pageSize')) : initialFilters.pageSize || DEFAULT_PAGE_SIZE,
    };
  ...
  const updateUrl = useCallback((newFilters: Partial<InventoryFilterOptions>) => {
    ...
    if (newFilters.quantityRange?.min !== undefined) params.set('minQty', newFilters.quantityRange.min.toString());
    if (newFilters.quantityRange?.max !== undefined) params.set('maxQty', newFilters.quantityRange.max.toString());
