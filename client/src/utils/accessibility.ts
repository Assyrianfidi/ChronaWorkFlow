...
export const validateHeadingLevels = (container: HTMLElement): string[] => {
  ...
  headings.forEach((heading, index) => {
    ...
    if (index === 0 && currentLevel !== 1) {
      errors.push(`First heading should be h1, found ${heading.tagName.toLowerCase()}`);
    }
    
    if (currentLevel > previousLevel + 1) {
      errors.push(`Heading level skipped: from h${previousLevel} to ${heading.tagName.toLowerCase()}`);
    }
  ...
  images.forEach((img, index) => {
    ...
    if (alt === null) {
      ...
      if (img.getAttribute('role') !== 'presentation') {
        errors.push(`Image ${index + 1} has empty alt but no role="presentation"`);
      }
    }
  });
...
export const validateFormAccessibility = (form: HTMLElement): string[] => {
  ...
  inputs.forEach((input, index) => {
    ...
    if (input.hasAttribute('required') && !input.getAttribute('aria-required')) {
      errors.push(`Required input ${index + 1} should have aria-required="true"`);
    }
    
    if (input.hasAttribute('aria-invalid') && !input.getAttribute('aria-describedby')) {
      errors.push(`Invalid input ${index + 1} should have aria-describedby pointing to error message`);
    }
  });
...
export const validateLandmarks = (container: HTMLElement): string[] => {
  const landmarks = container.querySelectorAll('[role="banner"], [role="navigation"], [role="main"], [role="complementary"], [role="contentinfo"], [role="search"], [role="form"]');
  const errors: string[] = [];
  ...
  if (!container.querySelector('[role="navigation"], nav')) {
    errors.push('Missing navigation landmark (role="navigation" or <nav> element)');
  }
...
export const runAccessibilityAudit = (containerId: string): {
  passed: boolean;
  errors: string[];
  warnings: string[];
} => {
  ...
  const allErrors = [...headingErrors, ...altTextErrors, ...formErrors, ...landmarkErrors];
...
export const detectScreenReader = (): boolean => {
  ...
  return hasScreenReader || navigator.userAgent.includes('NVDA') || 
