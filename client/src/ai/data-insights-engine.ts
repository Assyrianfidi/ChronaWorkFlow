...
export interface DataInsight {
  id: string;
  type: 'kpi_prediction' | 'cashflow_forecast' | 'anomaly_detection' | 'categorization' | 'recommendation';
  title: string;
  description: string;
...
export class AIDataInsightsEngine {
    ...
    const defaultRules: CategorizationRule[] = [
      {
        ...
        conditions: [
          { field: 'description', operator: 'contains', value: 'subscription', weight: 0.8 },
          { field: 'description', operator: 'contains', value: 'software', weight: 0.7 },
          { field: 'description', operator: 'contains', value: 'saas', weight: 0.9 },
      ...
      },
      {
        ...
        conditions: [
          { field: 'description', operator: 'contains', value: 'office', weight: 0.8 },
          { field: 'description', operator: 'contains', value: 'supplies', weight: 0.9 },
          { field: 'description', operator: 'contains', value: 'stationery', weight: 0.7 }
        ],
      ...
      },
      {
        ...
        conditions: [
          { field: 'description', operator: 'contains', value: 'travel', weight: 0.8 },
          { field: 'description', operator: 'contains', value: 'flight', weight: 0.9 },
          { field: 'description', operator: 'contains', value: 'hotel', weight: 0.9 },
          { field: 'description', operator: 'contains', value: 'taxi', weight: 0.7 }
        ],
  ...
  private async fetchExpenseData(): Promise<any[]> {
    ...
    return [
      { id: '1', category: 'Software', amount: 99, description: 'Software subscription' },
      { id: '2', category: 'Office Supplies', amount: 150, description: 'Office supplies' },
      { id: '3', category: 'Software', amount: 5000, description: 'Software subscription' }, // Anomaly
      { id: '4', category: 'Travel', amount: 200, description: 'Flight ticket' },
      { id: '5', category: 'Software', amount: 49, description: 'Software subscription' }
    ];
  }
    ...
    this.cashflowForecasts.forEach((forecast, period) => {
      if (forecast.riskLevel !== 'low') {
        const insight: DataInsight = {
          ...
          impact: forecast.riskLevel === 'critical' ? 'critical' : 
                  forecast.riskLevel === 'high' ? 'high' : 'medium',
          timestamp: new Date(),
          data: forecast,
          actionable: true,
  ...
  private calculateImpact(prediction: KPIPrediction): 'low' | 'medium' | 'high' | 'critical' {
    const variance = Math.abs(prediction.predictedValue - prediction.currentValue) / prediction.currentValue;
  ...
  private generateKPIActions(prediction: KPIPrediction): InsightAction[] {
    ...
    if (prediction.trend === 'decreasing') {
      actions.push({
        ...
        implementation: () => this.navigateTo(`/analytics/${prediction.metric}`)
      });
    }
  ...
  public categorizeTransaction(transaction: any): { category: string; confidence: number } {
  ...
  private evaluateCategorizationRule(rule: CategorizationRule, transaction: any): { category: string; confidence: number } {
    let totalScore = 0;
    let totalWeight = 0;
    ...
    rule.conditions.forEach(condition => {
      ...
      switch (condition.operator) {
        case 'equals':
          ...
          break;
        case 'contains':
          matches = fieldValue && fieldValue.toString().toLowerCase().includes(condition.value.toString().toLowerCase());
          break;
        case 'startsWith':
          matches = fieldValue && fieldValue.toString().toLowerCase().startsWith(condition.value.toString().toLowerCase());
          break;
        case 'endsWith':
          matches = fieldValue && fieldValue.toString().toLowerCase().endsWith(condition.value.toString().toLowerCase());
          break;
        case 'greaterThan':
          matches = fieldValue > condition.value;
          break;
        case 'lessThan':
          ...
          break;
        case 'regex':
          matches = fieldValue && new RegExp(condition.value).test(fieldValue.toString());
          break;
      }
  ...
  public provideCategorizationFeedback(transactionId: string, ruleId: string, correct: boolean): void {
  ...
  getExpenseAnomalies(): ExpenseAnomaly[] {
    return Array.from(this.expenseAnomalies.values()).sort((a, b) => b.detectedAt.getTime() - a.detectedAt.getTime());
  }
  ...
  updateCategorizationRule(ruleId: string, updates: Partial<CategorizationRule>): void {
    const rule = this.categorizationRules.get(ruleId);
    if (rule) {
...
class DataInsightsModel {
  ...
  private identifyFactors(metric: string, data: number[]): string[] {
    ...
    if (metric.includes('revenue')) {
      factors.push('Seasonal trends', 'Customer acquisition', 'Pricing changes');
  ...
  forecastCashflow(data: any[], period: 'daily' | 'weekly' | 'monthly' | 'quarterly'): CashflowForecast {
    ...
    return {
      ...
      riskLevel,
      keyDrivers,
      recommendations: this.generateCashflowRecommendations(riskLevel, keyDrivers)
    };
  }

  private calculateCashflowRisk(netCashflow: number, outflow: number): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private identifyCashflowDrivers(data: any[]): Array<{ category: string; impact: number; confidence: number }> {
    ...
    const totalImpact = Array.from(categoryImpact.values()).reduce((a, b) => a + b, 0);
  ...
  private generateCashflowRecommendations(riskLevel: string, drivers: Array<{ category: string; impact: number }>): string[] {
  ...
  detectExpenseAnomaly(expense: any): ExpenseAnomaly | null {
    ...
    if (variancePercent > 0.5) { // 50% variance threshold
      ...
      return {
        ...
        suggestedAction: this.generateAnomalySuggestion(severity, variancePercent)
      };
    }
  ...
  private calculateAnomalySeverity(variancePercent: number): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private identifyAnomalyFactors(expense: any): string[] {
    ...
    if (new Date(expense.date).getDay() === 0) { // Sunday
  ...
  private generateAnomalySuggestion(severity: string, variancePercent: number): string {
    switch (severity) {
      case 'critical':
...
export function useAIDataInsights() {
  ...
  const [kpiPredictions, setKPIPredictions] = React.useState(engine.getKPIPredictions());
  const [cashflowForecasts, setCashflowForecasts] = React.useState(engine.getCashflowForecasts());
  const [expenseAnomalies, setExpenseAnomalies] = React.useState(engine.getExpenseAnomalies());
  ...
  return {
    ...
    provideCategorizationFeedback: engine.provideCategorizationFeedback.bind(engine),
