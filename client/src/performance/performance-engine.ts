...
export class PerformanceEngine {
  ...
  private logApiPerformance(url: string, duration: number, status: number): void {
  ...
  private calculateMetrics(): PerformanceMetrics {
    ...
    // Core Web Vitals
    const fcpEntry = this.paintTimings.find(entry => entry.name === 'first-contentful-paint');
    ...
    const firstInputDelay = fidEntry ? fidEntry.processingStart - fidEntry.startTime : 0;

    const clsValue = this.layoutShiftEntries.reduce((sum, entry: any) => sum + entry.value, 0);
    const cumulativeLayoutShift = clsValue;

    // Custom metrics
    const pageLoadTime = navigation ? navigation.loadEventEnd - navigation.navigationStart : now - this.navigationStart;
  ...
  private calculateAverageApiResponseTime(): number {
    ...
    const apiCalls = this.resourceTimings.filter(resource => 
      resource.initiatorType === 'fetch' || resource.initiatorType === 'xmlhttprequest'
    );
    
    if (apiCalls.length === 0) return 0;
    
    const totalTime = apiCalls.reduce((sum, resource) => sum + resource.responseEnd - resource.requestStart, 0);
    return totalTime / apiCalls.length;
  }

  private calculateBundleSize(): number {
    // Calculate total bundle size from resource timings
    const scripts = this.resourceTimings.filter(resource => resource.initiatorType === 'script');
  ...
  private calculateTimeToInteractive(): number {
    ...
    const fcp = this.paintTimings.find(entry => entry.name === 'first-contentful-paint')?.startTime || 0;
  ...
  private generateReport(): void {
    ...
    if (this.config.monitoring.reportingEndpoint && Math.random() < this.config.monitoring.sampleRate) {
  ...
  private calculatePerformanceScore(metrics: PerformanceMetrics): number {
    ...
    const scores = {
      firstContentfulPaint: Math.max(0, 100 - (metrics.firstContentfulPaint / this.config.thresholds.firstContentfulPaint) * 100),
      largestContentfulPaint: Math.max(0, 100 - (metrics.largestContentfulPaint / this.config.thresholds.largestContentfulPaint) * 100),
      firstInputDelay: Math.max(0, 100 - (metrics.firstInputDelay / this.config.thresholds.firstInputDelay) * 100),
      cumulativeLayoutShift: Math.max(0, 100 - (metrics.cumulativeLayoutShift / this.config.thresholds.cumulativeLayoutShift) * 100),
      timeToInteractive: Math.max(0, 100 - (metrics.timeToInteractive / this.config.thresholds.timeToInteractive) * 100),
      pageLoadTime: Math.max(0, 100 - (metrics.pageLoadTime / this.config.thresholds.pageLoadTime) * 100)
    };
  ...
  private generateRecommendations(metrics: PerformanceMetrics): PerformanceRecommendation[] {
    ...
    if (metrics.firstContentfulPaint > this.config.thresholds.firstContentfulPaint) {
      recommendations.push({
        ...
        impact: 'high',
        effort: 'medium',
        implementation: 'Implement server-side rendering, optimize critical CSS, and reduce render-blocking resources.',
        resources: [
    ...
    if (metrics.largestContentfulPaint > this.config.thresholds.largestContentfulPaint) {
      recommendations.push({
        ...
        impact: 'high',
        effort: 'medium',
        implementation: 'Optimize images, preload important resources, and remove unnecessary third-party scripts.',
        resources: [
          'https://web.dev/optimize-lcp/',
    ...
    if (metrics.firstInputDelay > this.config.thresholds.firstInputDelay) {
      recommendations.push({
        ...
        impact: 'medium',
        effort: 'low',
        implementation: 'Break up long tasks, reduce JavaScript execution time, and use web workers.',
        resources: [
          'https://web.dev/fid/',
    ...
    if (metrics.cumulativeLayoutShift > this.config.thresholds.cumulativeLayoutShift) {
      recommendations.push({
        ...
        impact: 'medium',
        effort: 'medium',
        implementation: 'Include size attributes for images and videos, avoid inserting content above existing content.',
        resources: [
          'https://web.dev/cls/',
    ...
    if (metrics.memoryUsage > 50 * 1024 * 1024) { // 50MB
      recommendations.push({
        ...
        impact: 'low',
        effort: 'medium',
        implementation: 'Clean up event listeners, use object pooling, and avoid memory leaks.',
        resources: [
  ...
  private detectViolations(metrics: PerformanceMetrics): PerformanceViolation[] {
