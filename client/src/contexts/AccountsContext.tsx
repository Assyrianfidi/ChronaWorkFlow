...
// Reducer
const accountsReducer = (state: AccountsState, action: AccountsAction): AccountsState => {
  switch (action.type) {
    case 'SET_LOADING':
      return {
        ...state,
...
const AccountsContext = createContext<{
  ...
  createAccount: (accountData: Omit<Account, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>;
...
export const AccountsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Create account
  const createAccount = async (accountData: Omit<Account, 'id' | 'createdAt' | 'updatedAt'>): Promise<void> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const updateAccount = async (id: string, updates: Partial<Account>): Promise<void> => {
    try {
      ...
      dispatch({ type: 'UPDATE_ACCOUNT', payload: { id, updates: response.data.data } });
    } catch (error: any) {
  ...
  const adjustBalance = async (id: string, amount: number): Promise<void> => {
    try {
      ...
      const message = error.response?.data?.message || 'Failed to adjust balance';
      dispatch({ type: 'SET_ERROR', payload: message });
      throw error;
    }
  };
