...
export default function Reconciliation() {
  ...
  const bankAccounts = accounts.filter((account: Account) =>
  ...
  const filteredBankTransactions = bankTransactions.filter((txn: BankTransaction) => {
    const matchesAccount = !selectedAccount || txn.accountId === selectedAccount;
  ...
  const reconciledCount = filteredBankTransactions.filter(txn => txn.isReconciled).length;
  const unreconciledCount = filteredBankTransactions.filter(txn => !txn.isReconciled).length;
  const totalAmount = filteredBankTransactions.reduce((sum, txn) => sum + parseFloat(txn.amount), 0);
      ...
      <div className="grid gap-6 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-3">
            ...
            <div className="text-2xl font-semibold text-chart-2" data-testid="text-reconciled-count">
              {reconciledCount}
            </div>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-3">
            ...
            <div className="text-2xl font-semibold text-destructive" data-testid="text-unreconciled-count">
              {unreconciledCount}
            </div>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-3">
            ...
            <div className="text-2xl font-semibold tabular-nums" data-testid="text-net-change">
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <div className="text-2xl font-semibold" data-testid="text-match-rate">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-4">
                  ...
                  />
                  <Button disabled={!selectedFile || !selectedAccount} data-testid="button-upload-bank-file">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className="text-muted-foreground">{unreconciledCount}</div>
                </div>
              </div>
              <div className="pt-4 border-t">
                <Button variant="outline" className="w-full" disabled={!selectedAccount}>
        ...
        <CardContent>
          {filteredBankTransactions.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              {selectedAccount ? "No bank transactions found for this account." : "Select a bank account to view transactions."}
            </div>
          ) : (
            <Table>
              <TableHeader>
                ...
                {filteredBankTransactions.map((txn: BankTransaction) => {
                  ...
                  return (
                    <TableRow key={txn.id} data-testid={`bank-transaction-row-${txn.id}`}>
                      ...
                      <TableCell className={`text-right tabular-nums font-medium ${isPositive ? 'text-chart-2' : 'text-destructive'}`}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    ...
                                    <div className="space-y-1 text-sm">
                                      ...
                                      <div><strong>Reference:</strong> {txn.referenceNumber || 'None'}</div>
                                      <div><strong>Type:</strong> {isPositive ? 'Deposit' : 'Withdrawal'}</div>
                                    </div>
                                  </div>
                                  <div>
                                    <h4 className="font-medium mb-2">Account Details</h4>
                                    <div className="space-y-1 text-sm">
                                      <div><strong>Account:</strong> {account?.code} - {account?.name}</div>
                                      <div><strong>Amount:</strong> ${Math.abs(parseFloat(txn.amount)).toFixed(2)}</div>
                                      <div><strong>Status:</strong> {txn.isReconciled ? 'Reconciled' : 'Unreconciled'}</div>
                                      {txn.matchedTransactionId && (
                                        <div><strong>Matched With:</strong> Transaction ID</div>
                                      )}
                                    </div>
                                  </div>
                                </div>
                          ...
                          {!txn.isReconciled ? (
                            ...
                            <Button variant="ghost" size="icon" data-testid={`button-unmatch-${txn.id}`}>
