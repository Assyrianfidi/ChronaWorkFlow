...
export default function LoginPage() {
  ...
  useEffect(() => {
    const savedVariant = localStorage.getItem('accubooks:login-variant') as 'A' | 'B' | 'C' || 'A';
    ...
    switch (variant) {
      case 'A': // Glass-Morph
        return {
          ...
          container: 'max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8 items-center',
          heroSection: 'hidden lg:flex flex-col items-center justify-center p-8',
          cardSection: 'variant-glass rounded-xl p-8 shadow-elevated',
          card: 'bg-transparent border-0 shadow-none p-0',
          input: 'bg-white/70 border-white/20 text-text-dark placeholder:text-muted focus:ring-2 focus:ring-accent',
          button: 'bg-primary hover:bg-primary/90 text-white shadow-lg hover:shadow-xl transition-all duration-300',
      ...
      case 'B': // Minimal Professional
        return {
          ...
          card: 'bg-transparent border-0 shadow-none p-0',
          input: 'bg-white border-gray-200 text-text-dark placeholder:text-muted focus:ring-2 focus:ring-accent',
          button: 'bg-primary hover:bg-primary/90 text-white shadow-md hover:shadow-lg transition-all duration-300',
          link: 'text-accent hover:text-accent/80 font-medium',
          text: 'text-text-dark',
          mutedText: 'text-muted'
        };
      case 'C': // Bold Product
        return {
          ...
          container: 'max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-0 items-center min-h-screen',
          heroSection: 'hero-gradient text-white flex flex-col items-center justify-center p-8 lg:p-12 rounded-tl-xl rounded-bl-xl',
          cardSection: 'variant-bold-card rounded-tr-xl rounded-br-xl p-8 lg:p-12',
          card: 'bg-transparent border-0 shadow-none p-0',
          input: 'bg-gray-50 border-gray-200 text-text-dark placeholder:text-muted focus:ring-2 focus:ring-accent',
          button: 'bg-accent hover:bg-accent/90 text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-300',
      ...
      <div className={classes.container}>
        ...
        {(variant === 'A' || variant === 'C') && (
          <div className={classes.heroSection}>
            ...
            />
            <h2 className={`text-3xl lg:text-4xl font-bold mb-4 ${variant === 'C' ? 'text-white' : classes.text}`}>
              AccuBooks
            </h2>
            <p className={`text-center ${variant === 'C' ? 'text-gray-300' : classes.mutedText} max-w-md`}>
              Enterprise financial operations reimagined. Manage your business with confidence and precision.
            </p>
          </div>
        )}
        ...
        <section aria-label="Sign in" className={classes.cardSection}>
          {/* Theme Selector */}
          <div className="flex items-center justify-between mb-6" role="radiogroup" aria-label="Theme variant">
            ...
            <form onSubmit={handleSubmit} aria-label="Login form">
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="email" className={`text-sm font-semibold ${classes.text}`}>
                    Email Address
                  </Label>
                  <InputWithIcon
                    id="email"
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="password" className={`text-sm font-semibold ${classes.text}`}>
                      Password
                    </Label>
                    <Link 
                      to="/forgot-password" 
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    ...
                    />
                    <span className={`text-sm ${classes.mutedText}`}>Remember me</span>
                  </label>
                </div>
          ...
          <footer className="mt-8 pt-6 border-t border-gray-200">
            <div className="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-6 text-sm">
              <Link to="/privacy" className={classes.mutedText + ' hover:text-accent transition-colors'}>
                Privacy Policy
              </Link>
              <span className="hidden sm:inline text-gray-400">•</span>
              <Link to="/terms" className={classes.mutedText + ' hover:text-accent transition-colors'}>
                Terms of Service
              </Link>
            </div>
