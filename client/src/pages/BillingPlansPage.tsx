...
const BillingPlansPage: React.FC = () => {
  ...
  const currentPlanCode =
    subscription?.plan?.code?.replace("_annual", "") ?? null;
  const currentPlan = plans?.find(
    (p) => p.code === currentPlanCode || p.code === `${currentPlanCode}_annual`,
  );
  ...
  const getPlanFeatures = (plan: any) => {
    ...
    if (plan.allowMultiEntityConsolidation)
      features.push("Multi-entity consolidation");
