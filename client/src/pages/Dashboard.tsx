...
import { KPICard, MetricCard, StatusCard } from "../components/ui/EnterpriseCards"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "../components/ui/EnterpriseCards"
...
const Dashboard: React.FC = () => {
  ...
  const getQuickActions = () => {
    ...
    return [...baseActions, ...(roleBasedActions[user?.role as keyof typeof roleBasedActions] || [])]
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Here's what's happening with your {user?.role === "admin" ? "system" : user?.role === "accountant" ? "accounting" : "business"} today.
            </p>
          </div>
                ...
                {quickActions.map((action, index) => {
                  ...
                  return (
                    <button
                      key={index}
                      className={cn(
                        "group flex items-center gap-3 p-4 rounded-xl border-2 border-dashed transition-all duration-200",
                        "hover:border-solid hover:shadow-md hover:scale-[1.02]",
                        action.color === "primary" && "hover:border-primary-300 hover:bg-primary-50",
                        action.color === "success" && "hover:border-success-300 hover:bg-success-50",
                        action.color === "warning" && "hover:border-warning-300 hover:bg-warning-50",
                        action.color === "error" && "hover:border-error-300 hover:bg-error-50",
                        action.color === "info" && "hover:border-info-300 hover:bg-info-50"
                      )}
                    >
                      <div className={cn(
                        "w-10 h-10 rounded-lg flex items-center justify-center",
                        action.color === "primary" && "bg-primary-100 text-primary-600 group-hover:bg-primary-200",
                        action.color === "success" && "bg-success-100 text-success-600 group-hover:bg-success-200",
                        action.color === "warning" && "bg-warning-100 text-warning-600 group-hover:bg-warning-200",
                        action.color === "error" && "bg-error-100 text-error-600 group-hover:bg-error-200",
                        action.color === "info" && "bg-info-100 text-info-600 group-hover:bg-info-200"
                      )}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <MetricCard
                title="Performance"
                metrics={[
                  { label: "API Response", value: "124ms", color: "text-success-600" },
                  { label: "Database", value: "99.9%", color: "text-success-600" },
