...
const mockAuditLogs: AuditLog[] = [
  ...
  },
  {
    ...
    resource: 'CUSTOMER',
    resourceId: 'CUST-015',
    details: 'Attempted to delete customer with active invoices - Action blocked',
  ...
  },
  {
    ...
    action: 'LOGIN',
    resource: 'AUTH',
    details: 'Failed login attempt - Invalid credentials for admin@accubooks.com',
            ...
            <Select value={actionFilter} onValueChange={setActionFilter}>
              ...
              <SelectContent>
                ...
                {getUniqueValues('action').map(action => (
                  <SelectItem key={action || 'unknown'} value={action || 'unknown'}>{action}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <Table>
              ...
              <TableBody>
                {filteredLogs.map((log) => {
                  ...
                  const ActionIcon = actionConfig[log.action as keyof typeof actionConfig]?.icon || Activity
        ...
        <Card className="border-red-200 bg-red-50">
          ...
          <CardContent>
            <p className="text-red-700">
              {filteredLogs.filter(log => log.severity === 'CRITICAL').length} critical security events detected. 
