          ...
          {pricingTiers.map((tier) => (
            ...
            >
              ...
              <div className={`p-6 ${tier.popular ? 'pt-10' : ''}`}>
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${tier.popular ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                  {tier.icon}
                </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                {pricingTiers.map((tier) => (
                  <th key={tier.id} className="text-center py-4 px-4 font-medium text-foreground">
                    {tier.name}
                  </th>
                ))}
              </tr>
            ...
            {faqs.map((faq, index) => (
              ...
              >
                ...
                >
                  ...
                  <HelpCircle className={`w-5 h-5 text-muted-foreground transition-transform ${expandedFaq === index ? 'rotate-180' : ''}`} />
                </button>
                {expandedFaq === index && (
