...
export default async function ProfilePage() {
  ...
  const hasPassword = user.accounts.some((account: any) => account.provider === 'credentials');
  const socialAccounts = user.accounts.filter((account: any) => account.provider !== 'credentials');
                    ...
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center">
                        <span className="text-gray-500">
                          {account.provider === 'google' ? 'G' : account.provider === 'github' ? 'GH' : account.provider[0].toUpperCase()}
                        </span>
                      </div>
                      <div className="ml-4">
