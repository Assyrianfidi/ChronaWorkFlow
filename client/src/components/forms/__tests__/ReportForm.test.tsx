...
const LARGE_FILE = new File(['a'.repeat(6 * 1024 * 1024)], 'large.pdf', { type: 'application/pdf' });
const IMAGE_FILE = new File(['test'], 'test.png', { type: 'image/png' });
const INVALID_FILE = new File(['test'], 'test.exe', { type: 'application/octet-stream' });
...
vi.mock('next/image', () => ({
  __esModule: true,
  default: (props: any) => <img {...props} data-testid="mocked-image" alt={props.alt || ''} />,
}));
...
describe('ReportForm', () => {
  it('renders core form fields correctly', () => {
    ...
    expect(screen.getByLabelText(/make this report public/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /save/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /cancel/i })).toBeInTheDocument();
    expect(screen.getByTestId('__REPORTFORM_SELECT_CATEGORY__')).toBeInTheDocument();
    expect(screen.getByTestId('__REPORTFORM_SELECT_REASON__')).toBeInTheDocument();
    expect(screen.getByTestId('__REPORTFORM_SELECT_PRIORITY__')).toBeInTheDocument();
  });
  ...
  it('applies initial values for core fields', () => {
    ...
    expect(screen.getByTestId('__REPORTFORM_SELECT_CATEGORY__')).toHaveValue('approved');
    expect(screen.getByTestId('__REPORTFORM_SELECT_REASON__')).toHaveValue('high');
  ...
  it('submits valid form data', async () => {
    ...
    render(<ReportForm {...defaultProps} onSubmit={mockOnSubmitLocal} initialValues={{
      title: '',
      description: '',
      status: 'draft',
      priority: 'medium',
    ...
    await user.type(screen.getByPlaceholderText(/enter report title/i), 'Test Report');
    await user.type(screen.getByPlaceholderText(/enter report description/i), 'This is a test report description');
    fireEvent.change(screen.getByTestId('__REPORTFORM_SELECT_CATEGORY__') ?? screen.getByTestId('status-native'), { target: { value: 'in_review' } });
    fireEvent.change(screen.getByTestId('__REPORTFORM_SELECT_REASON__') ?? screen.getByTestId('priority-native'), { target: { value: 'high' } });
    fireEvent.change(screen.getByTestId('__REPORTFORM_SELECT_PRIORITY__') ?? screen.getByTestId('dueDate'), { target: { value: '2025-11-20' } });
    await user.click(screen.getByRole('button', { name: /save|submit/i }));

    try {
      await waitFor(() => expect(mockOnSubmitLocal).toHaveBeenCalled(), { timeout: 1500 });
    } catch (err) {
    ...
    expect(mockOnSubmitLocal).toHaveBeenCalledWith(expect.objectContaining({
      title: 'Test Report',
