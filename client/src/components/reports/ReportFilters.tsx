...
export function ReportFilters({ onFilterChange, className }: ReportFiltersProps) {
  const [searchParams, setSearchParams] = useSearchParams();
  const [filters, setFilters] = useState<ReportFilters>({
    search: searchParams.get('search') || '',
    status: searchParams.get('status') || 'all',
    startDate: searchParams.get('startDate') || format(subMonths(new Date(), 1), 'yyyy-MM-dd'),
  ...
  const handleDateChange = (date: Date | undefined, name: 'startDate' | 'endDate') => {
    if (date) {
        ...
        <div className="flex w-full items-center space-x-2 md:w-auto">
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {filters.startDate ? format(parseISO(filters.startDate), 'MMM dd, yyyy') : 'From'}
              </Button>
            </PopoverTrigger>
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                <CalendarIcon className="mr-2 h-4 w-4" />
                {filters.endDate ? format(parseISO(filters.endDate), 'MMM dd, yyyy') : 'To'}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0">
              <Calendar
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          {filters.startDate && (
            <Button
              ...
              className="h-8"
            >
              {format(parseISO(filters.startDate), 'MMM d')} - {format(parseISO(filters.endDate || new Date().toISOString()), 'MMM d, yyyy')}
              <X className="ml-2 h-3 w-3" />
            </Button>
          )}
        </div>
