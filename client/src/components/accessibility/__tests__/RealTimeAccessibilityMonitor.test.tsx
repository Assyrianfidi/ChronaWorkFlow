...
} from '../RealTimeAccessibilityMonitor';
...
const TestComponent: React.FC = () => {
  ...
  return (
    <div>
      <div data-testid="monitoring-status">{isMonitoring ? 'monitoring' : 'not-monitoring'}</div>
      <div data-testid="violations-count">{violations.length}</div>
      <div data-testid="errors-count">{metrics.violations.errors}</div>
      <button onClick={startMonitoring} data-testid="start-btn">Start</button>
      <button onClick={stopMonitoring} data-testid="stop-btn">Stop</button>
      <button onClick={scanPage} data-testid="scan-btn">Scan</button>
      <button onClick={() => fixViolation('test-id')} data-testid="fix-btn">Fix</button>
    </div>
  );
};
...
describe('RealTimeAccessibilityMonitor', () => {
  ...
  it('provides accessibility monitor context', () => {
    ...
    expect(screen.getByTestId('monitoring-status')).toHaveTextContent('not-monitoring');
