...
const VoiceCommandEngineContext = React.createContext<VoiceCommandEngineContextType | null>(null);
...
export const VoiceCommandEngine: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [isListening, setIsListening] = useState(false);
  const [recognizedCommands, setRecognizedCommands] = useState<VoiceCommand[]>(BUILTIN_COMMANDS);
  ...
  const processVoiceCommand = useCallback(async (transcript: string, confidence: number) => {
    ...
    if (matchedCommand) {
      ...
      announceToScreenReader('Command not found. Say "help" for available commands.');
    ...
    try {
      ...
      switch (command.action) {
        ...
        case 'cancel':
          ...
          if (cancelButton && (cancelButton.textContent?.includes('Cancel') || cancelButton.textContent?.includes('Close'))) {
            cancelButton.click();
          }
          break;
        ...
        case 'search':
          ...
          if (searchInput) {
            searchInput.focus();
            const searchTerm = transcript.replace(/search for|find|look for/gi, '').trim();
  ...
  const playSuccessSound = () => {
    const audio = new Audio('data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBjiS2Oy9diMFl2+z5N17PwU');
    audio.play().catch(() => {});
  };
  ...
  const triggerExport = () => {
    const exportButton = document.querySelector('button[aria-label*="export" i], button:contains("Export")') as HTMLButtonElement;
    if (exportButton) {
      exportButton.click();
    } else {
  ...
  const showVoiceCommandsHelp = () => {
    const commandsText = allCommands.map(cmd => 
