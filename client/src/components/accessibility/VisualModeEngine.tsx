...
interface VisualModeContextType {
  ...
  updateCustomMode: (modeId: string, settings: Partial<VisualModeSettings>) => void;
...
const VisualModeContext = React.createContext<VisualModeContextType | null>(null);
...
export const VisualModeEngine: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [currentMode, setCurrentMode] = useState<VisualMode | null>(BUILTIN_MODES[0]);
  const [customModes, setCustomModes] = useState<VisualMode[]>([]);
  const [settings, setSettings] = useState<VisualModeSettings>(BUILTIN_MODES[0].settings);
  
  // Refs to prevent infinite loops
  const isInitialized = useRef(false);
  const mediaQueryRefs = useRef<{ query: MediaQueryList; handler: () => void }[]>([]);

  // Memoize available modes to prevent unnecessary recalculations
  const availableModes = React.useMemo(() => [...BUILTIN_MODES, ...customModes], [customModes]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      root.style.setProperty('--border-radius', `${settings.borderRadius * 0.5}rem`);
      root.style.setProperty('--shadows', settings.shadows ? '1' : '0');
      
      // Apply image filters
      root.style.setProperty('--img-brightness', settings.imageFilters.brightness.toString());
      root.style.setProperty('--img-contrast', settings.imageFilters.contrast.toString());
      root.style.setProperty('--img-saturation', settings.imageFilters.saturation.toString());
      root.style.setProperty('--img-grayscale', settings.imageFilters.grayscale ? '1' : '0');
  ...
  const createCustomMode = useCallback((mode: Omit<VisualMode, 'id'>) => {
    ...
    // Save to localStorage with error handling
    try {
      const savedModes = JSON.parse(localStorage.getItem('custom-visual-modes') || '[]');
  ...
  const updateCustomMode = useCallback((modeId: string, newSettings: Partial<VisualModeSettings>) => {
    setCustomModes(prev => prev.map(mode => 
      mode.id === modeId 
    ...
    try {
      const savedModes = JSON.parse(localStorage.getItem('custom-visual-modes') || '[]');
      const modeIndex = savedModes.findIndex((m: VisualMode) => m.id === modeId);
      if (modeIndex >= 0) {
        savedModes[modeIndex].settings = { ...savedModes[modeIndex].settings, ...newSettings };
        localStorage.setItem('custom-visual-modes', JSON.stringify(savedModes));
      }
    } catch (error) {
  ...
  const deleteCustomMode = useCallback((modeId: string) => {
    ...
    try {
      const savedModes = JSON.parse(localStorage.getItem('custom-visual-modes') || '[]');
      const filteredModes = savedModes.filter((m: VisualMode) => m.id !== modeId);
      localStorage.setItem('custom-visual-modes', JSON.stringify(filteredModes));
    } catch (error) {
  ...
  const updateSettings = useCallback((newSettings: Partial<VisualModeSettings>) => {
    setSettings(prev => ({ ...prev, ...newSettings }));
  }, []);
  ...
  const detectSystemPreferences = useCallback(() => {
    try {
      ...
      if (prefersHighContrast) {
        const highContrastMode = BUILTIN_MODES.find(mode => mode.id === 'high-contrast');
        if (highContrastMode) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Load custom modes
      const savedCustomModes = JSON.parse(localStorage.getItem('custom-visual-modes') || '[]');
      ...
      const mediaQueries = [
        { query: window.matchMedia('(prefers-reduced-motion: reduce)'), handler: detectSystemPreferences },
        { query: window.matchMedia('(prefers-contrast: high)'), handler: detectSystemPreferences },
        { query: window.matchMedia('(prefers-color-scheme: dark)'), handler: detectSystemPreferences }
      ];
