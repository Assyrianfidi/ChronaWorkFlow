...
describe('AnalyticsEngine', () => {
    ...
    function TestComponent() {
      ...
      return (
        <div>
          ...
          <div>Result: {result ? JSON.stringify(result).substring(0, 50) : 'none'}</div>
  ...
  it('generates forecasts', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Forecast: linear_regression')).toBeInTheDocument();
    });
  });
...
describe('AnalyticsEngine Performance', () => {
  it('disables animations in low performance mode', async () => {
    ...
    function TestComponent() {
      ...
      return (
        <div>
          <div>Animation: {dashboard?.widgets[0]?.config?.animation ? 'enabled' : 'disabled'}</div>
        </div>
      );
    }
  ...
  it('adapts to user experience mode', async () => {
    ...
    function TestComponent() {
      ...
      return (
        <div>
          <div>Interactive: {dashboard?.widgets[0]?.config?.interactive ? 'enabled' : 'disabled'}</div>
        </div>
      );
    }
