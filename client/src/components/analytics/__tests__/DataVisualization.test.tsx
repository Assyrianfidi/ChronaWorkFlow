...
import { DataVisualization } from '../AnalyticsEngine';
...
describe('DataVisualizationComponent', () => {
  ...
  it('renders canvas element', () => {
    ...
    expect(canvas).toHaveClass('border', 'border-gray-200', 'rounded', 'cursor-crosshair');
  });
...
describe('BaseChart', () => {
  ...
  it('shows grid when enabled', () => {
    ...
    expect(mockCanvasContext.beginPath).toHaveBeenCalledTimes(expect.any(Number));
  });
...
describe('DashboardLayout', () => {
  ...
  it('renders empty state when no widgets', () => {
    ...
    expect(screen.getByText('Add visualizations to build your dashboard')).toBeInTheDocument();
  });
  ...
  it('uses grid layout by default', () => {
    ...
    const container = screen.getByText('Test Dashboard').closest('div')?.parentElement;
  ...
  it('formats last updated timestamp', () => {
    ...
    const dashboardWithTime = { ...mockDashboard, lastUpdated: testTime.getTime() };
