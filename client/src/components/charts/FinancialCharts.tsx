...
import { useCashFlow, useTopExpenses, CashFlowData, ExpenseCategory } from '@/hooks/useFinancialData';
  ...
  if (active && payload && payload.length) {
    return (
      <div className={`p-3 rounded-lg shadow-lg border ${
        ...
        <p className={`font-semibold mb-2 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
          {label}
        </p>
...
export function CashFlowChart({ months = 6, height = 300 }: CashFlowChartProps) {
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Tooltip
                ...
                cursor={{ fill: isDark ? 'rgba(55, 65, 81, 0.3)' : 'rgba(243, 244, 246, 0.8)' }}
              />
              <Bar
                dataKey="inflow"
                name="Inflow"
...
export function TopExpensesChart({ period = 'current-month', height = 300 }: TopExpensesChartProps) {
  const { themeName, resolvedMode } = useEnhancedTheme();
  const colors = getThemeColors(themeName, resolvedMode);
...
export function MiniTrendChart({ data, color = '#22c55e', height = 40 }: MiniTrendChartProps) {
