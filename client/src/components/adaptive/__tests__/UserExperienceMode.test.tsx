...
import { UserExperienceModeProvider, useUserExperienceMode, UXModeSelector, UXCustomSettings } from '../UserExperienceMode';
...
describe('UserExperienceModeProvider', () => {
  ...
  it('saves mode to localStorage when changed', async () => {
    const TestComponent = () => {
      ...
      return (
        <div>
          ...
          <button onClick={() => setMode({ id: 'power-user', name: 'Power User', description: 'Fast mode', icon: '⚡', theme: 'dark', density: 'compact', animations: 'minimal', accessibility: 'standard', notifications: 'minimal', shortcuts: true, tooltips: false, sounds: false })}>
    ...
    await waitFor(() => {
      expect(localStorageMock.setItem).toHaveBeenCalledWith('ux-mode', 'power-user');
    });
  });
...
describe('UXModeSelector', () => {
  ...
  it('switches mode when clicking on mode option', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Power User').closest('button')).toHaveClass('border-blue-500');
    });
  });
});
...
describe('useUserExperienceMode hook', () => {
  it('throws error when used outside provider', () => {
    ...
    expect(screen.getByText(/useUserExperienceMode must be used within UserExperienceModeProvider/)).toBeInTheDocument();
  });
