...
describe('InventoryTable', () => {
  ...
  it('renders inventory items', () => {
    ...
    mockInventoryItems.forEach(item => {
      ...
      expect(screen.getByText((item.quantityOnHand || 0).toString())).toBeInTheDocument();
    });
  });
  ...
  it('handles row selection', () => {
    ...
    expect(mockOnSelectItem).toHaveBeenCalledWith(mockInventoryItems[0].id, true);
  });
  ...
  it('handles row expansion', () => {
    ...
    expect(screen.getByText(mockInventoryItems[0].description || '')).toBeInTheDocument();
  });
  ...
  it('displays correct stock status', () => {
    ...
    const lowStockItem = mockInventoryItems.find(item => item.sku === 'SKU-002');
    ...
    const outOfStockItem = mockInventoryItems.find(item => item.sku === 'SKU-003');
    const outOfStockRow = screen.getByText(outOfStockItem!.name).closest('tr');
    expect(within(outOfStockRow!).getByText(/out of stock/i)).toBeInTheDocument();
  });

  it('handles bulk actions', () => {
    const mockOnBulkAction = vi.fn();
    const selectedItems = new Set([mockInventoryItems[0].id, mockInventoryItems[1].id]);
  ...
  it('displays correct item details in expanded row', () => {
    ...
    expect(screen.getByText(`Barcode: ${firstItem.barcode}`)).toBeInTheDocument();
    expect(screen.getByText(`Location: ${firstItem.location}`)).toBeInTheDocument();
    expect(screen.getByText(`Unit Cost: $${firstItem.unitCost.toFixed(2)}`)).toBeInTheDocument();
    expect(screen.getByText(`Unit Price: $${firstItem.unitPrice.toFixed(2)}`)).toBeInTheDocument();
  });
});
