...
interface WorkflowDefinition {
  ...
  name: string;
  description: string;
  category: 'business' | 'technical' | 'approval' | 'notification' | 'integration';
  version: string;
  steps: WorkflowStep[];
...
interface WorkflowTemplate {
  ...
  description: string;
  category: string;
  steps: Omit<WorkflowStep, 'id' | 'status' | 'metadata' | 'position' | 'connections'>[];
...
export const SmartWorkflow: React.FC<{
      ...
      steps: [
        ...
        },
        {
          ...
          config: {
            ...
            conditions: [
              { field: 'managerDecision', operator: 'equals', value: 'approved' }
            ]
          }
        }
      ],
      ...
      steps: [
        {
          ...
          config: {
            ...
            actions: [{ type: 'monitor', config: { metrics: ['error_rate', 'response_time'] } }]
          }
        },
        {
          ...
          config: {
            ...
            conditions: [
              { field: 'impact', operator: 'greater', value: 5 }
            ]
          }
        },
        {
          ...
          config: {
            ...
            actions: [{ type: 'notification', config: { channels: ['email', 'slack'] } }]
          }
        },
        {
      ...
      steps: [
        {
          ...
          config: {
            ...
            actions: [{ type: 'api', config: { endpoint: '/api/users/create' } }]
          }
        },
        {
  ...
  const handleStepUpdate = useCallback((stepId: string, updates: Partial<WorkflowStep>) => {
  ...
  const handleAddStep = useCallback((template: Omit<WorkflowStep, 'id' | 'status' | 'metadata' | 'position' | 'connections'>) => {
    if (!workflow || !onWorkflowChange || readOnly) return;
  ...
  const handleApplyTemplate = useCallback((template: WorkflowTemplate) => {
    ...
    const newWorkflow: WorkflowDefinition = {
      ...
      variables: Object.fromEntries(
        Object.entries(template.variables).map(([key, config]) => [key, config.default])
      ),
      settings: {
    ...
    return (
      ...
      >
        <div className="flex items-center mb-2">
          <div className={`w-3 h-3 rounded-full ${stepTypeColors[step.type]} mr-2`} />
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              value={step.name}
              onChange={(e) => handleStepUpdate(selectedStep, { name: e.target.value })}
          ...
          <div>
            ...
            <select
              value={step.type}
              onChange={(e) => handleStepUpdate(selectedStep, { type: e.target.value as any })}
              className="w-full p-2 border rounded"
              disabled={readOnly}
            >
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => handleStepUpdate(selectedStep, {
    ...
    return (
      <div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">Create a new workflow or select an existing one</p>
          <button
