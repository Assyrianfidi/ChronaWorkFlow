...
describe('AIPoweredAssistant', () => {
  ...
  it('displays welcome message with actions', () => {
    ...
    expect(screen.getByText('Generating reports and dashboards')).toBeInTheDocument();
  ...
  it('sends user message and receives response', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/I can help you create an automation rule/)).toBeInTheDocument();
    });
  });
  ...
  it('executes actions from assistant response', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/✅ Create Automation completed successfully/)).toBeInTheDocument();
    });
  });

  it('handles action execution errors', async () => {
    const mockCreateRule = vi.fn().mockRejectedValue(new Error('Creation failed'));
    ...
    await waitFor(() => {
      expect(screen.getByText(/❌ Failed to execute Create Automation/)).toBeInTheDocument();
    });
  });
  ...
  it('shows suggestions in assistant responses', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Create a data backup automation')).toBeInTheDocument();
    });
  });
  ...
  it('clicks suggestions to populate input', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Show me how to create an automation')).toBeInTheDocument();
    });
...
describe('AIPoweredAssistant AI Features', () => {
  it('recognizes different intents', async () => {
    ...
    for (const testCase of testCases) {
      ...
      await waitFor(() => {
        const responses = screen.getAllByText(/I can help you|Let me check|I understand|I'm here to help|I understand you're/);
  ...
  it('extracts entities from messages', async () => {
    ...
    const input = screen.getByPlaceholderText('Ask me anything...');
    fireEvent.change(input, { target: { value: 'Create automation for 5 users on 01/15/2024' } });
    fireEvent.click(screen.getByText('Send'));

    await waitFor(() => {
      expect(screen.getByText('Create automation for 5 users on 01/15/2024')).toBeInTheDocument();
    });
  });
  ...
  it('learns from interactions', async () => {
    ...
    const messages = ['create automation', 'make automation', 'set up automation'];
