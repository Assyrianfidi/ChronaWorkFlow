...
import { WebhookManager, useWebhookManager } from '../WebhookManager';
...
describe('WebhookManager', () => {
  ...
  it('updates webhook endpoint', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('First Endpoint Name: Updated Webhook')).toBeInTheDocument();
  ...
  it('toggles webhook endpoint', async () => {
    function TestComponent() {
      ...
      return (
        <div>
          <div>First Endpoint Active: {endpoints[0]?.active ? 'yes' : 'no'}</div>
  ...
  it('gets events with filters', async () => {
    function TestComponent() {
      ...
      const handleTriggerAndFilter = async () => {
        ...
        await triggerEvent('payment.completed', 'Stripe', { paymentId: 'pay_123' });
  ...
  it('gets deliveries with filters', async () => {
    function TestComponent() {
      const { getDeliveries, createEndpoint, triggerEvent } = useWebhookManager();
  ...
  it('retries failed delivery', async () => {
    function TestComponent() {
      const { createEndpoint, triggerEvent, getDeliveries, retryDelivery } = useWebhookManager();
      ...
      const handleFailAndRetry = async () => {
        try {
          ...
          setTimeout(async () => {
            const deliveries = getDeliveries({ webhookId: endpoint.id, status: 'failed' });
  ...
  it('creates endpoint from template', async () => {
    function TestComponent() {
      ...
      const handleCreateFromTemplate = async () => {
        ...
        if (userTemplate) {
          await createFromTemplate(userTemplate.id, 'My User Webhook', 'https://myapp.com/webhooks/users');
          setCreated(true);
        }
      };
  ...
  it('calculates analytics', async () => {
    function TestComponent() {
      const { getAnalytics, createEndpoint, triggerEvent } = useWebhookManager();
...
describe('WebhookManager Integration', () => {
  ...
  it('handles performance mode adaptations', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Endpoints in Performance Mode: 0')).toBeInTheDocument();
    });
  });
});
  ...
  it('handles template creation errors gracefully', async () => {
    function TestComponent() {
      ...
      const handleCreateFromInvalidTemplate = async () => {
        try {
          await createFromTemplate('invalid-template-id', 'Test Webhook', 'https://example.com/webhook');
        } catch (err) {
          setError(err instanceof Error ? err.message : 'Unknown error');
        }
      };
      ...
      return (
        <div>
          <div>Error: {error}</div>
          <button onClick={handleCreateFromInvalidTemplate}>Create from Invalid Template</button>
        </div>
      );
    }
...
describe('WebhookManager Features', () => {
    ...
    function TestComponent() {
      ...
      return (
        <div>
          <div>Created: {created ? 'yes' : 'no'}</div>
          <div>Endpoints: {endpoints.length}</div>
          <button onClick={handleCreateWithRetryPolicy}>Create with Linear Retry</button>
        </div>
      );
    }
  ...
  it('supports webhook transformations', async () => {
    function TestComponent() {
      ...
      return (
        <div>
          <div>Created: {created ? 'yes' : 'no'}</div>
          <div>Transformations: {endpoints[0]?.transformations.length || 0}</div>
          <button onClick={handleCreateWithTransformations}>Create with Transformations</button>
        </div>
      );
    }
  ...
  it('provides comprehensive event tracking', async () => {
    function TestComponent() {
      ...
      const handleTrackEvents = async () => {
        // Trigger different types of events
        await triggerEvent('user.created', 'AccuBooks', { userId: '1', email: 'user1@example.com' });
        await triggerEvent('user.updated', 'AccuBooks', { userId: '2', email: 'user2@example.com' });
        await triggerEvent('payment.completed', 'Stripe', { paymentId: 'pay_123', amount: 1000 });

        const tracking = {
          totalEvents: events.length,
          userEvents: events.filter(e => e.type.startsWith('user.')).length,
          paymentEvents: events.filter(e => e.type.startsWith('payment.')).length,
      ...
      return (
        <div>
          <div>Event Tracking: {eventTracking ? JSON.stringify(eventTracking) : 'none'}</div>
  ...
  it('provides delivery statistics', async () => {
    function TestComponent() {
      const { createEndpoint, triggerEvent, getDeliveries } = useWebhookManager();
      ...
      return (
        <div>
          <div>Delivery Stats: {deliveryStats ? JSON.stringify(deliveryStats) : 'none'}</div>
