...
interface WebhookManagerContextType {
  ...
  createEndpoint: (
    endpoint: Omit<
      WebhookEndpoint,
      "id" | "createdAt" | "updatedAt" | "deliveryHistory" | "statistics"
    >,
...
const WebhookManagerContext =
  React.createContext<WebhookManagerContextType | null>(null);
...
class WebhookEngine {
        ...
        case "contains":
          return (
            ...
            (filter.caseSensitive
              ...
              filter.caseSensitive ? filter.value : filter.value.toLowerCase(),
            )
          );
        case "not_contains":
          return (
  ...
  private async createDelivery(
    ...
    const payload = await this.applyTransformations(
      event.data,
    ...
    if (endpoint.secret) {
      headers["X-Webhook-Signature"] = await this.generateSignature(
        payload,
        endpoint.secret,
        endpoint.security.signatureAlgorithm,
      );
    }
  ...
  private async applyTransformations(
    ...
    for (const transformation of transformations) {
      switch (transformation.type) {
        case "field_mapping":
          transformed = this.applyFieldMapping(
            transformed,
            transformation.config.mappings,
          );
          break;
        case "data_enrichment":
          transformed = this.applyDataEnrichment(
            transformed,
            transformation.config.enrichments,
          );
          break;
        case "format_conversion":
          transformed = this.applyFormatConversion(
            transformed,
            transformation.config,
          );
          break;
        case "custom":
          transformed = await this.applyCustomTransformation(
            transformed,
            transformation.config,
          );
          break;
  ...
  private updateEndpointStatistics(endpoint: WebhookEndpoint): void {
    ...
    stats.errorRate =
      stats.totalDeliveries > 0
        ? stats.failedDeliveries / stats.totalDeliveries
        : 0;

    // Calculate average delivery time
    const deliveries = Array.from(this.deliveries.values()).filter(
      (d) => d.webhookId === endpoint.id && d.response?.duration,
    );

    if (deliveries.length > 0) {
      stats.averageDeliveryTime =
        deliveries.reduce((sum, d) => sum + d.response!.duration, 0) /
        deliveries.length;
    }
  }
  ...
  getDeliveries(filters?: {
    ...
    if (filters?.webhookId) {
      filtered = filtered.filter(
        (delivery) => delivery.webhookId === filters.webhookId,
      );
    }
...
export const WebhookManager: React.FC<{ children: React.ReactNode }> = ({
  ...
  const updateEndpoint = useCallback(
    async (id: string, updates: Partial<WebhookEndpoint>): Promise<void> => {
      if (!engineRef.current) return;
  ...
  const deleteEndpoint = useCallback(async (id: string): Promise<void> => {
    ...
    setDeliveries((prev) =>
      prev.filter((delivery) => delivery.webhookId !== id),
    );
  }, []);
  ...
  const toggleEndpoint = useCallback(async (id: string): Promise<void> => {
    ...
    setEndpoints((prev) =>
      prev.map((endpoint) =>
        endpoint.id === id
          ? { ...endpoint, active: !endpoint.active, updatedAt: Date.now() }
          : endpoint,
      ),
    );
  }, []);
  ...
  const createFromTemplate = useCallback(
    async (
      ...
      const endpoint = await engineRef.current.createFromTemplate(
        templateId,
        name,
        url,
      );
