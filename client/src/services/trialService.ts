...
class TrialService {
  ...
  async completeMilestone(milestoneType: string): Promise<{
    success: boolean;
    data: { milestone: string; completed: boolean };
  }> {
    const response = await fetch(
      `${API_BASE}/trial/milestone/${milestoneType}`,
      {
        method: "POST",
        headers: this.getAuthHeaders(),
      },
  ...
  async convertTrial(planType: string): Promise<{
    success: boolean;
    data: { converted: boolean; newPlan: string };
  }> {
  ...
  getMilestoneInfo(milestoneType: string): {
    name: string;
    description: string;
    points: number;
  } {
