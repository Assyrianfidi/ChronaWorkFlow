...
class OfflineService {
  ...
  // Cache management
  async setCache(key: string, data: any, ttl: number = 300000): Promise<void> { // Default 5 minutes
    try {
  ...
  async getQueueStatus(): Promise<{ pending: number; failed: number }> {
    try {
      ...
      const failed = queue.filter(item => item.retryCount >= this.maxRetries).length
  ...
  async cleanupExpiredCache(): Promise<void> {
    try {
      const keys = Object.keys(localStorage).filter(k => k.startsWith(this.cacheName))
      let cleaned = 0
