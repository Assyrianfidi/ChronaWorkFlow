...
const Dashboard: React.FC = () => {
  ...
  const getRoleSpecificMetrics = (): Record<string, string> => {
    switch (user?.role) {
      case 'CFO':
        return {
          revenue: '$2,456,789',
  ...
  const renderDashboardContent = () => {
    switch (activeTab) {
      case 'overview':
        return <OverviewDashboard userRole={user?.role || ''} metrics={metrics} />
      case 'financial':
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Welcome back! Here's your financial summary for {user?.role}.</p>
      </div>
...
const OverviewDashboard = ({ userRole, metrics }: { userRole: string, metrics: Record<string, string> }) => {
  return (
    <div>
      {/* Key Metrics */}
