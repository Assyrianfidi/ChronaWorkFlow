 ...
 */
export type EventCategory = 'auth' | 'dashboard' | 'scenario' | 'forecast' | 'trust' | 'error' | 'performance';
...
export interface ScenarioEvent extends BaseAnalyticsEvent {
    category: 'scenario';
    action: 'created' | 'edited' | 'deleted' | 'duplicated' | 'comparison_viewed' | 'wizard_started' | 'wizard_completed' | 'wizard_abandoned';
    metadata?: {
        scenarioType?: string;
        stepNumber?: number;
...
export interface TrustEvent extends BaseAnalyticsEvent {
    category: 'trust';
    action: 'calculation_explainer_opened' | 'calculation_explainer_closed' | 'calculation_step_expanded' | 'assumptions_panel_viewed' | 'assumption_clicked' | 'confidence_indicator_hovered' | 'confidence_indicator_clicked';
    metadata?: {
        confidenceScore?: number;
...
export interface ErrorEvent extends BaseAnalyticsEvent {
    category: 'error';
    action: 'runtime_error' | 'api_error' | 'render_error' | 'validation_error' | 'network_error';
    metadata: {
        errorType: string;
        errorMessage: string;
...
export interface PerformanceEvent extends BaseAnalyticsEvent {
    ...
    action: 'page_load' | 'api_call' | 'forecast_calculation' | 'chart_render' | 'component_mount';
    metadata: {
        duration: number;
        endpoint?: string;
        componentName?: string;
...
export type AnalyticsEvent = AuthEvent | DashboardEvent | ScenarioEvent | ForecastEvent | TrustEvent | ErrorEvent | PerformanceEvent;
