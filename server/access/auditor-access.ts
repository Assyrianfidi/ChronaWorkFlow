...
export type AuditorAccessTokenPayload = {
  v: 1;
  typ: 'AUDITOR_REPORT_ACCESS';
  jti: string;
  sub: string;
  companyId: string;
  validFrom: string;
...
export class AuditorAccessService {
  ...
  async grantAccess(input: {
    ...
    const jti = makeJti(`aud:${input.subject.externalId}:${input.scope.companyId}:${input.scope.from.toISOString()}:${input.scope.to.toISOString()}`);

    const payload: AuditorAccessTokenPayload = {
      v: 1,
      typ: 'AUDITOR_REPORT_ACCESS',
      jti,
  ...
  async verifyToken(input: { token: string; correlationId: string }): Promise<AuditorAccessTokenPayload> {
    const decoded = jwt.verify(input.token, getSecret(), { algorithms: ['HS256'] }) as any;

    if (!decoded || decoded.typ !== 'AUDITOR_REPORT_ACCESS' || decoded.v !== 1) {
      throw new Error('Invalid auditor access token');
    }
    ...
    return decoded as AuditorAccessTokenPayload;
  }
}
