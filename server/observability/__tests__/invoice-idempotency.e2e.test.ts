...
function resolveTestDatabaseUrl(): string | null {
  ...
  const fromProcess = [
    process.env.INVOICE_E2E_DATABASE_URL,
...
async function checkDbPreflight(dbUrl: string): Promise<{ ok: boolean; diag: any }> {
  ...
  try {
    ...
    const tableCheckSql = `
      ...
      AND table_name IN ('companies', 'users', 'customers', 'invoices', 'invoice_items')
    `;
    ...
    const diag = {
      ...
      invoice_items: foundTables.has("invoice_items") ? "invoice_items" : null,
    };

    await client.end();

    const required = ["companies", "users", "customers", "invoices", "invoice_items"];
...
describe("invoices idempotency (db-backed)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!testDbUrl) {
      throw new Error("No DATABASE_URL found for invoice idempotency E2E test");
    }

    const preflight = await checkDbPreflight(testDbUrl);
    if (!preflight.ok) {
      throw new Error(
        `Invoice idempotency E2E test requires migrated DB schema. DATABASE_URL=${testDbUrl} diag=${JSON.stringify(preflight.diag)}`
      );
    }
  ...
  it("concurrent replay: one invoice write, consistent responses, items created once", async () => {
    ...
    await db.insert(s.companies).values({
      id: companyId,
      name: "Test Company Invoice Idem",
      email: `invoice-idem-${Date.now()}@test.local`,
      createdAt: new Date(),
      updatedAt: new Date(),
    });
    ...
    await db.insert(s.users).values({
      id: userId,
      username: `invoice-idem-user-${Date.now()}`,
      email: `invoice-idem-user-${Date.now()}@test.local`,
      password: "hashed",
      name: "Invoice Idem User",
      role: "admin",
      createdAt: new Date(),
      updatedAt: new Date(),
    });
    ...
    const idempotencyKey = "idem-invoice-test-1";
    ...
    const invoices = await db
      .select()
      .from(s.invoices)
      .where(and(eq(s.invoices.companyId as any, companyId), eq(s.invoices.customerId as any, customerId)));
    ...
    const items = await db
      .select()
      .from(s.invoiceItems)
      .where(eq(s.invoiceItems.invoiceId, invoices[0].id));

    expect(items.length).toBe(2);

    await db.delete(s.invoiceItems).where(eq(s.invoiceItems.invoiceId, invoices[0].id));
    await db.delete(s.invoices).where(eq(s.invoices.id, invoices[0].id));
